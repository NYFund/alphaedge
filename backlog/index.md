...
| P2 | ⬜ | [股期前N檔篩選的fallback修正.md](股期前N檔篩選的fallback修正.md) | `FuturesPriceUpdater.resolve_stock_futures_products()` 排不出流動性時，會把 `top_n` 這個上限拿掉、退回全部 320 檔標的池（請求量放大 16 倍，只印一行 WARNING）；改成收緊或明確中止，並處置 2026-09-18 中斷留下的 CAF 半段行情 | 1 / 3 項 ✅（S2，2026-09-18）；S1、S3 ⬜ | **無前置相依，S1 可直接開工**。**S2 已完成**：2026-09-18 使用者裁示刪除 CAF 的 8,500 列與 1,730 個中繼檔，`get_top_liquid_products(20)` 已由 `['CAF']` 回到 `[]`，S1 的修正因此可在真實情境下驗證；S3 相依 S1，會改寫 [暫緩工作彙整.md](暫緩工作彙整.md) S4 的暖身做法。與 S4 不同軸：S4 是「要不要爬股期」（等策略立項），本文件是「爬的時候不要被放大成 320 檔」（現在就能修）。2026-09-18 立項——當日預設的 `python -m tasks.update_db` 實際踩中，對 320 檔從 2015 年起回補、5.5 小時內卡死排在後面的 `futures_chip`／`fs`／`mrr` 等 target；`tasks/update_db.py` 已於同日把 `futures_stock_price` 移出 `all`／`no_tick`，不再列入本文件 |
...
| P3 | ⏸ | [暫緩工作彙整.md](暫緩工作彙整.md) | 集中各文件因外部條件暫緩的工作：FinMind 連網冒煙腳本、券商分點 metadata 的 `last_attempted_date` 與 NO_DATA 延遲重試、股票期貨行情前 N 檔回補、平盤下放空限制與每日可當沖清單、漲跌停改用公告值 | 0 / 6 項（S1~S6 ⏸） | **全部暫緩，無可開工項**。S1~S3 等 FinMind 帳號升級（目前 `register`，無券商分點權限，2026-09-16 以真實 API 確認）；S4 等開始開發股期策略，**解除時不可直接跑 `--target futures_stock_price`**，須先暖身（見該文件 S4）——但 **S4 的暖身做法已於 2026-09-18 標記過期**，改寫前不要照著執行，見 [股期前N檔篩選的fallback修正.md](股期前N檔篩選的fallback修正.md) S3；S5~S6 等交易所公告資料源盤點（處置／警示股、每日可當沖清單、漲跌停公告值），兩者共用同一批來源，建議同批施作。2026-09-17 由 `DAO重構後續收斂.md`、`券商分點NO_DATA的metadata語意.md`、`爬蟲缺口回補與非交易日批次清理.md` 的暫緩項集中而成；2026-09-18 併入 `docs已載明但未實作的缺口盤點.md` 的 S8、S9（該文件其餘八項已完成並移出）。四份原文件皆已刪除，完成紀錄留在 git 歷史 |
