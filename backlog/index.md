...
| 優先級 | 狀態 | 檔案 | 說明 | 進度 | 相依 |
|:------:|:----:|------|------|------|------|
| P1 | 🔄 | [健檢第五輪收斂.md](健檢第五輪收斂.md) | 第五輪全專案健檢：回測做多路徑記帳口徑（除權息、鎖漲停買進）、已入庫的歷史資料錯誤（`price` 重播日、籌碼自營商欄、`stock_id` 前導 0）、期貨日夜盤整併與時段完整性、半份資料防線的 cleaner 缺口、前端容器、文件與規則漂移 | 0 / 30 項 ✅；Phase4-3、Phase5-1 🔄 | **無前置相依**。Phase1-7、Phase1-8、Phase3-7 需使用者裁示（Phase4-3 的刪除裁示已於 2026-09-15 定案）；Phase2-3、Phase3-2 的重爬與 [暫緩工作彙整.md](暫緩工作彙整.md) S4（股期行情回補，解除時）共用交易所請求節奏，不要同時跑；Phase2-6 與 Phase3-3 同型、建議同批 |
