...
| 優先級 | 狀態 | 檔案 | 說明 | 進度 | 相依 |
|:------:|:----:|------|------|------|------|
| P1 | 🔄 | [健檢第五輪收斂.md](健檢第五輪收斂.md) | 第五輪全專案健檢：回測做多路徑記帳口徑（除權息、鎖漲停買進）、已入庫的歷史資料錯誤（`price` 重播日、籌碼自營商欄、`stock_id` 前導 0）、期貨日夜盤整併與時段完整性、半份資料防線的 cleaner 缺口、前端容器、文件與規則漂移 | 28 / 29 項 ✅；Phase1-7 ⏸（裁示不收斂，策略已刪）| **無前置相依**。2026-09-17 全數施作完畢：程式與測試 `pytest -m "not slow"` 1,159 → 1,219 passed 且無 skip、回歸雙線通過、三支護欄腳本結束碼 0；資料修正（`price` 兩個重播日、`漲跌價差` 162 萬列、早期籌碼 37.6 萬列、代號前導 0 15 列）驗證查詢皆歸零。Phase1-7 依裁示不處理並刪除該策略；原 Phase3-7（DolphinDB 殘留）整條移出本文件，盤點清單已搬進 [台股tick改用TimescaleDB.md](台股tick改用TimescaleDB.md) 的 Phase5-2。重爬與 [暫緩工作彙整.md](暫緩工作彙整.md) S4 共用交易所請求節奏，不要同時跑 |
