...
| P2 | ⬜ | [策略分層重構.md](策略分層重構.md) | 把策略的「選標的（Alpha）」與「決定部位大小（Portfolio Construction）」由同一個方法拆成兩層：新增 `core/portfolio/`（`Signal` 模型、`sizing.py` 由 `core/backtest/models/` 搬入、`Signal → Order` 的 builder），策略只實作 `generate_*_signals()`，`check_*_signal()` 下沉到基底；跨策略仲裁 `aggregation.py` 亦歸此層。命名一律對齊業界通稱（portfolio construction／signal generation／portfolio aggregation／position sizing），但**刻意不採用 `PortfolioTarget`（隱含本專案沒有的 target-weight 再平衡語意）與 `RiskModel`（在事前風控與因子風險模型之間有歧義）**。**引擎對策略的契約完全不變**（`check_*_signal(quotes) -> List[Order]`），`Backtester` 與 `LiveTrader` 一行都不用改 | 0 / 7 項（S1~S7 ⬜） | **必須排在 [實盤下單架構規劃.md](實盤下單架構規劃.md) 的 Phase3-4 與 Phase4-1 之前**：Phase4-1 會往 `BaseStrategy` 加 `live_ready`／`live_schedule`，Phase3-4 會動 `Backtester`，`ForeignSellShortDayTradeStrategy` 兩邊都會改到——順序反了就要重構兩次、回歸雙線跑兩次，且兩次都動策略層回歸，出問題分不出成因。S1（`sizing.py` 搬家）無前置相依可最先做，import 點實測只有 2 處。**核心驗收是 `run_regression.sh` 零變動**（只搬結構不改邏輯）。2026-09-19 立項，起因：`BaseStockStrategy` 在實盤會 import `core.backtest.models.sizing`（方向相反）、`max_holdings` 被 sizer 與 `Backtester` 各檢查一次、期貨走保證金約束卻與股票的資金切分沒有共同介面 |
