...
| P2 | ⏸ | [DAO重構後續收斂.md](DAO重構後續收斂.md) | DAO 資料存取層完成後的收尾：`query_df` 改用 cursor 讓查詢失敗不 rollback 共用連線、保證金歷史回補跳過已入庫公告、7 個以 `to_sql` 建表的測試改走 `dao_factory`、FinMind 連網冒煙腳本、PostgreSQL 遷移計畫重新盤點、清掉既有 F841；另含實跑發現的 FinMind 券商分點錯誤被算成無資料、入庫摘要誤計新寫入 | 7 / 8 項 ✅，S4 ⏸ | **只剩 S4 暫緩**：FinMind 連網冒煙腳本，使用者裁示暫緩（帳號無券商分點權限），解除條件為帳號升級或確實需要連網檢查；其餘 7 項已完成並驗收；S7 發現時 FinMind 帳號等級為 `register`，無券商分點資料集權限。2026-09-16 由 DAO 重構的完成檢視與正式 DB 實跑立項，設計見 [資料存取層](../docs/dev/data-access-layer.md) |
