...
- **做法**（使用者手動）：
  ...
  4. 在本文件記下完成日期、使用的 shioaji 版本（目前 `uv.lock` 為 `1.3.3`）。
- **產出**：本步驟章節末尾的完成紀錄。
...
- **做法**：
  ...
  - ⚠️ **借券（SBL）要先升 shioaji**：鎖定版 1.3.3 的 `StockOrderCond` 沒有 `SBLShort`
    （2026-09-19 Phase1-1 實測），本步驟開工前先確認版本，否則借券那條路徑只能拋出。升版工作見 [Shioaji升級至1.7.md](Shioaji升級至1.7.md)。
