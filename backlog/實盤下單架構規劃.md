...
> 與「讀寫」拆兩個檔**，判定以純函式放 `core/portfolio/aggregation.py`（該目錄由
> [策略分層重構.md](策略分層重構.md) 建立；落點判準是「回答什麼問題」——資金分配與同標的仲裁
...
| Phase4-1 | 日頻執行時點契約（`ExecutionTiming`） | `core/strategies/base.py`、`core/models/base/order.py`、`core/models/stock/quote.py`、`core/models/futures/quote.py` | 回歸零變動；未宣告 `live_ready` 的策略拒絕進入實盤；開盤段讀 `quote.close` 會拋出；**`live_schedule` 與 `bar_execution_order` 矛盾時拒絕啟動** | ⬜ | 相依 D1、D0、Phase1-2。2026-09-19 補兩者的一致性檢查：分屬不同段落時實際順序由段落決定，宣告相反會靜默改掉交易順序 |
...
引擎層        Backtester                  **LiveTrader**（市場無關，無子類）
                ...
                │                              │
部位建構層    **core/portfolio/**（由 [策略分層重構.md](策略分層重構.md) 建立；
                                   該份另含 signal.py／sizing.py／construction.py）
...
├── portfolio/                          # ← 由 backlog/策略分層重構.md 建立
...
- **做法**：在 `scripts/check_layer_deps.py` 做以下調整：
  - `_LAYER_RULES` 新增：
    ...
    - `("core.portfolio", 4, "部位建構層", False)`：**已於 2026-09-19 由 [策略分層重構.md](策略分層重構.md) S1 完成登記，本步驟不必再加**。實際白名單比原先規劃寬：可 import `core.models`／`core.utils`／`core.config`／`core.api`，**不可 import `core.backtest`／`core.live`／`core.strategies`**；另因 `FuturesPortfolioConstructor` 需要 `FuturesMarginConfig`，會出現 `core.portfolio` → `core.managers` 的同層邊（F 區，刻意）。跨策略仲裁的判定（`aggregation.py`）也在這一層。
  ...
  - **預期會出現的「同層互相 import」**（腳本只列出、不影響結束碼，但要在本步驟先登記理由，避免之後被當成新問題）：
    ...
    - **已存在（策略分層重構完成後）**：`core.strategies.stock.base` → `core.portfolio.sizing`／
...
- **驗證方式**：`tests/live/test_risk_manager.py`，每條規則各一個觸發與未觸發案例；`tests/live/test_trading_mode.py`：非法轉移（自動升級）拋出、降級後重啟仍是降級狀態；純判定的規則要能**不建立 `PreTradeRiskManager` 實例**就單獨測。
- **相依**：Phase3-2。
...
- **相依**：Phase0-4；**[策略分層重構.md](策略分層重構.md)**——該份已於 2026-09-19 完成（S1~S8），`core/strategies/` 與 `core/portfolio/` 的落點皆已定案，本步驟可直接開工。
- **注意**：這是本文件唯一會動到回測引擎的步驟，建議單獨一個 commit，方便回退。
- **與分層重構的交界（2026-09-19 定案）**：`max_holdings` 的兩處檢查**經比對後確認不等價，兩處都保留**，見[策略分層重構.md](策略分層重構.md) S7 的完成紀錄。兩者回答的是不同問題：`EqualWeightSizer.size()` 是**資金要切成幾份**（訊號階段，且張數不足 1 張的候選不佔名額），`Backtester.check_max_holdings()` 是**這張單送出去會不會讓帳戶超過上限**（逐單階段，看即時持倉數；未成交的單不增加持倉，後面的單因此仍可能被放行）。本步驟照現況把**引擎側**的判定搬進 `order_preprocess`，portfolio 側留在 sizer，**不合併**。
...
- **相依**：D1、D0、Phase1-2；**[策略分層重構.md](策略分層重構.md)**——該份已於 2026-09-19 完成，`BaseStrategy` 與 `ForeignSellShortDayTradeStrategy` 的分層已定案，本步驟可直接在新結構上加實盤欄位。**`live_ready`／`live_schedule` 加在 `BaseStrategy`，實盤獨有的訊號語意分歧落在 `generate_*_signals()` 的覆寫**，不是 `check_*_signal()`——後者現在由基底提供，覆寫它會把分層打回原形。
...
> **前置：[策略分層重構.md](策略分層重構.md) 已於 2026-09-19 完成**（原立項時要求排在 Phase3-4 與 Phase4-1 之前，已滿足）。
> 那份把策略的 Alpha 與 Portfolio Construction 拆成兩層，會動到 `core/strategies/` 三個基底、
> 三支策略與 `Backtester`。本文件的 Phase4-1 同樣要改 `BaseStrategy`（加 `live_ready`／
> `live_schedule`／`live_tag`）、Phase3-4 同樣要動 `Backtester`，`ForeignSellShortDayTradeStrategy`
> 兩邊都會改到。順序反過來的話，同一批檔案重構兩次、回歸雙線跑兩次，而且**兩次都動策略層的回歸，
> 出問題時分不出是哪一次造成的**。其餘 Phase 不受影響，可並行。
