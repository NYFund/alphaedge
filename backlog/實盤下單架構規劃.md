...
| D4 | 一個帳戶跑幾支策略 | **第一階段同一帳戶、同一商品類別只跑一支策略**。券商部位沒有策略標記，多策略要做本地歸屬帳，複雜度高。`custom_field` 只收英數字 6 字元，放不下 `strategy_name`（例如 `Momentum-1` 含連字號），故策略另外宣告 `live_tag`（Phase4-1） | 多策略共用時，對帳無法判斷差異屬於哪一支 | 待定 |
...
- **做法**：
  ...
  - 資料表：
    ...
    | 表 | 主鍵 | 用途 |
    |----|------|------|
    | `live_run` | `run_id` | 每次啟動一筆：策略、模式（simulation／production）、`dry_run`、phase、起訖時間、結束原因 |
...
- **做法**：
  ...
  - `recover(run_date)`：啟動時讀取當日未終結的委託，以 `refresh_order_status()` 比對後接管。
    ...
    - `PENDING_SUBMIT` 且沒有 seqno（在 `place_order()` 回傳前崩潰）的委託，以「`custom_field`＋標的＋買賣別＋價格＋數量＋送單時間窗」到券商當日委託中比對；比對到就接管，**比對到多筆時不猜，標為需人工確認並停止開新倉**。
...
- **做法**（以 D1 建議方案為準）：
  - `BaseStrategy` 新增：
    ...
    - `live_tag: str`：寫進 Shioaji `custom_field` 的策略代號，啟動時驗證為英數字且不超過 6 字元（D4）。
...
- **做法**：
  ...
  - `is_market_open(today)`：盤前沒有今天的 DB 資料，既有 `MarketCalendar` 的兩種判斷（查 `price` 表、查當日 tick）都用不上。改用券商端資訊判斷（例如合約檔的更新日期是否為今天），實作前先以模擬環境在休市日與開市日各核對一次；判斷不出來時**拒絕啟動**，不預設為開市。
