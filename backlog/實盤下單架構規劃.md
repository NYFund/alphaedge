...
| Phase7-1 | 模擬環境端到端演練（連續 5 個交易日）                        | 本文件（演練紀錄）                                                                                                                                                                        | 對應 Abstract 驗收標準 1、2、4、5                                                                                                                                      |  ⛔  | **2026-09-23 第 1 天三個段落全部失敗、當日作廢**（帳戶總權益算成 0 → 額度檢查拒絕啟動；根因與兩個缺口見章節紀錄，修完才能續跑）。2026-09-22 演練前準備完成並以 launchd 排程啟動（`scripts/launchd/rehearsal_schedule.py`），演練交易日 9/23、9/24、9/29、9/30、10/1。相依 Phase4-6~Phase4-8、Phase5-5、Phase6-2、Phase6-5、Phase6-6 |
...
- **做法**：在 `scripts/check_layer_deps.py` 做以下調整：
  ...
  - **預期會出現的「同層互相 import」**（腳本只列出、不影響結束碼，但要在本步驟先登記理由，避免之後被當成新問題）：
    - `core.live.datafeed` → `core.datafeed.base`：`LiveDataFeed` 繼承 `BaseDataFeed`，因為策略的 `setup_apis(feed)` 型別就是它。該契約已於 2026-09-22 搬到中立的 `core/datafeed/`（見 [架構重構與冗餘收斂.md](架構重構與冗餘收斂.md) Phase1-1），兩邊同層。
...
>
> **待辦（修完才能續跑演練）**：
> 1. 模擬環境下帳戶總權益為 0 時的處置（比照期貨保證金的兩層檢查，或另議）。
>    **這等於在模擬環境放掉一道真實守門，需裁示。**
> 2. `__unattributed__` 部位計入總權益的口徑。
> 3. 兩者都動 `core/live/`，須排在當日最後一個段落（台北 14:35）之後施作。
...
- **做法**：
  ...
  - `core/market/tw/market_calendar.py` 的 `check_shioaji_has_tick()` 會實際呼叫 Shioaji 查 tick，**不在本步驟範圍**，維持原樣。
    ...
    > 該函式後由 [架構重構與冗餘收斂.md](架構重構與冗餘收斂.md) Phase1-3 於 2026-09-22 刪除（零呼叫點），`market_calendar.py` 也因此不再 import shioaji。
- **產出**：上列檔案。
