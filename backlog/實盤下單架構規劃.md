...
| Phase7-5 | 並行段落互相標記 CRASHED，並靜默解除帳戶層 halt | `core/dao/tw/live_trade_dao.py` 的 `mark_crashed_runs()`、`core/live/trader.py`、`core/live/after_close.py` | 兩個段落時窗重疊時，後啟動者不得標記前者為 CRASHED；前次 `REDUCE_ONLY` 必須讀得回來 |  ⬜  | **2026-09-24 演練第 2 天發現，每個交易日都會發作**。相依：無 |
