...
| Phase6-1 | 融資融券與現股當沖下單                                       | `core/broker/tw/shioaji_broker.py`、測試 | 券源不足或不可當沖的標的在送單前被擋                                                                                                                                   | 🔄 | 2026-09-22 送單前資格檢查（當沖 `day_trade`、融券券源）與 15 條測試完成；**剩模擬環境各送一筆**（融券、借券、當沖先賣，`test_short_sell_round_trip`，需盤中）。相依 Phase2-4、Phase3-3、D3 |
