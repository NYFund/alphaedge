...
- **背景／問題（立項當時，2026-09-17；下列各點**現已全部解除**，保留是為了說明本文件為何存在）**：
  專案當時已經有「策略開發 → 爬蟲 pipeline → 回測」三段，但**沒有實盤下單**。
  - `run.py --mode live` 呼叫後直接拋 `NotImplementedError`。
  - Shioaji 相關程式只有三支 `core/utils/` 工具，都是早期留下來的（已於 Phase7-3 刪除）：
...
- **驗收標準**：
  1. 以**實盤入口**（目前 `python run.py --mode live --broker shioaji --simulation`；[回測與實盤入口拆分及架構收斂.md](回測與實盤入口拆分及架構收斂.md) 會換成 `python -m apps.live`，**本條驗收不綁入口名稱**）可以在 Shioaji 模擬環境跑以下兩條路徑，各連續 5 個交易日，
     每筆委託、成交、每日部位快照都寫進 `tw_trading.db`，收盤後對帳零差異。

     **兩條線各自獨立宣告通過，不必互相等**（2026-09-25 裁示，理由見〈附：股票線與期貨線的分線〉）：

     - **1S（股票線）**：一支股票日頻策略。
     - **1F（期貨線）**：一支期貨策略。

     兩線的行程本來就是分開的——每個交易日六個獨立行程（股票／期貨 × 開盤／尾盤／盤後），
     期貨行程崩潰不會影響股票行程。**但程式碼是共用的**，故分線只解除「進度互卡」，
     不解除「改動互相影響」（見該附錄）。
...
| Phase1-1 | 下單執行參數 Enum                                            | `core/utils/constant/`、`core/utils/__init__.py`、`tests/test_order_state_parity.py`                                                                                                    | 與 `shioaji.constant` 同名 Enum 的值逐一比對                                                                                                                           |  ✅  | 2026-09-19 完成。**實測發現 shioaji 1.3.3 的 `StockOrderCond` 沒有 `SBLShort`**（只有 Cash／MarginTrading／ShortSelling），借券賣出要升版才送得出去，已列為 ratchet 並影響 Phase2-4、Phase6-1。**2026-09-21 升 1.7.5 後已解除**：`SBLShort` 自 1.7.2 起支援，ratchet 已移除、mapper 照常轉換（Shioaji 升級至 1.7，已完成並移出）                                                                                                                                                       |
...
| Phase4-9 | 盤後以券商已實現損益校正成本估算 | `core/models/base/execution.py`、`core/broker/tw/shioaji_account_query.py`、`core/broker/tw/shioaji_broker.py`、`core/live/after_close.py`、`core/live/factory.py`、`core/dao/tw/live_trade_dao.py`、測試 | 期貨 fee＋tax 與股票推得的實際成本，與本地估算逐日比對並落地 | 🔄 | 2026-09-22 程式與 11 條測試完成；以當天模擬帳戶的真實交易驗證期貨估算與券商扣款一致（243 對 243）；**剩股票那一側的實跑**（要本地有開倉成交，Phase7-1 演練時自然產生）。相依 Phase4-6 |
...
| Phase5-4 | 日終強制動作：當沖回補、期貨夜盤跨日歸屬                     | `core/live/intraday/session_guard.py`                                                                                                                                                     | 模擬 13:20 仍有當沖部位時觸發回補                                                                                                                                      |  ✅  | 2026-09-21 判定與回補接線完成；2026-09-22 結案：實盤期貨只有日盤段落，夜盤成交不會發生，夜盤日期函式的呼叫端延到新增夜盤段落時接上。相依 Phase5-2 |
| Phase5-5 | 常駐部署與排程                                               | `docker-compose.yml`、`docs/deployment/live-deployment.md`、`core/live/termination.py`、實盤入口（`run.py`，入口拆分後為 `apps/live.py`）、測試 | 容器內跑完一個模擬交易日；SIGTERM 會先撤單再結束                                                                                                                       | 🔄 | 2026-09-22 compose `live` service、SIGTERM 收尾（退出碼 143）、部署與排程文件完成；**剩容器內以模擬環境跑一個完整交易日**。相依 Phase5-3 |
| Phase6-1 | 融資融券與現股當沖下單                                       | `core/broker/tw/shioaji_broker.py`、測試 | 券源不足或不可當沖的標的在送單前被擋                                                                                                                                   | 🔄 | **股票線**｜2026-09-22 送單前資格檢查（當沖 `day_trade`、融券券源）與 15 條測試完成；**剩模擬環境各送一筆**（融券、借券、當沖先賣，`test_short_sell_round_trip`，需盤中）。**待評估**：`api.contracts.info()` 的 `sbl_shortable`／`margin_shortable`／`below_ref_shortable`／`trading_suspended`／`disposition_level` 可取代「用券源張數推資格」（評估依據見本步驟章節），等實測結果再決定。相依 Phase2-4、Phase3-3、D3 |
| Phase6-2 | 指數期貨：代號轉換與開平倉別 | `core/broker/tw/shioaji_contract_resolver.py`、`shioaji_execution_handler.py`、`shioaji_account_query.py`、`shioaji_quote_stream.py`、`shioaji_order_mapper.py`、`shioaji_broker.py` | 四條路徑（成交回報、委託回報、部位、快照）都是 `{商品}{YYYYMM}`；平倉單送 `Cover`；模擬環境期貨成交回報的代號等於訂單代號 | ✅ | **期貨線**｜2026-09-22 完成：四條路徑統一為 `TX202610`、平倉送 `Cover`；模擬環境期貨成交回報代號等於訂單代號。原本的換月與保證金另立 Phase6-6、Phase6-5 |
| Phase6-3 | 股票期貨合約與乘數                                           | `core/broker/tw/shioaji_contract_resolver.py`                                                                                                                                             | 乘數取自合約並與 `futures_stock_universe` 交叉比對                                                                                                                     | ⏸ | **期貨線**｜2026-09-22 暫緩：實盤的期貨送單只解析指數期貨，股期沒有送單路徑，也沒有策略交易股期。**解除條件：開始開發股期策略時**（與 [暫緩工作彙整.md](暫緩工作彙整.md) S4 同一個條件）。相依 Phase6-2 |
| Phase6-4 | 盤中零股                                                     | `core/models/stock/order.py`、`core/broker/tw/shioaji_order_mapper.py`                                                                                                                    | 數量單位錯配（張當股）在建立委託時就拋出                                                                                                                               | ⏸ | **股票線**｜2026-09-22 暫緩：零股建構檢查與送單前拒絕已做；下游（金額、成本、部位、歸屬帳、對帳）全以張為單位，回測也不支援零股。**解除條件：有策略需要零股時**，整條路徑一起改單位。相依 Phase2-4 |
| Phase6-5 | 送單前的期貨保證金檢查 | `core/live/risk/margin_gate.py`（新）、`core/live/trader.py`、`core/live/factory.py`、`core/live/datafeed/tw/futures_live_datafeed.py`、`core/managers/futures/position_manager.py`、測試 | 可用保證金不足時不送單並寫事件；足夠時照常送出 | ✅ | **期貨線**｜2026-09-22 完成：策略層（帳戶可動用餘額，與回測同口徑）＋帳戶層（券商可用保證金；模擬環境回 0 時略過、正式環境擋單，使用者裁示）；實盤補注入保證金表 |
| Phase6-6 | 期貨換月：平舊月＋開新月兩腿 | `core/live/trader.py`、`core/live/datafeed/base.py`、`core/live/datafeed/tw/futures_live_datafeed.py`、`core/live/factory.py`、`core/broker/tw/shioaji_contract_resolver.py`、測試 | 換月日模擬環境實跑一次；平倉腿未成交時開倉腿放棄並寫事件 | 🔄 | **期貨線**｜2026-09-22 程式與 17 條測試完成（最後交易日前 1 個交易日換月，使用者裁示）；**剩換月日模擬環境實跑**：台指期 10 月契約最後交易日 2026-10-21，換月日為 **2026-10-20** 尾盤段。**待評估**：`api.contracts.info()` 的期貨 `last_trading_date` 可與專案日曆推算的最後交易日交叉驗證（評估依據見本步驟章節）。相依 Phase6-2、Phase6-5 |
| Phase7-1 | 模擬環境端到端演練（連續 5 個交易日）                        | 本文件（演練紀錄）                                                                                                                                                                        | 對應 Abstract 驗收標準 1、2、4、5                                                                                                                                      |  🔄  | **進行中，1S／1F 兩線各自計算**（見〈附：股票線與期貨線的分線〉）：Day-1（9/23）三段全部失敗當日作廢，Day-2（9/24）四個修正端到端驗證通過，9/25 中秋節休市日六段全部正常結束（跨休市日驗收達成）。9/29 起續跑。**⚠️ 至 9/25 為止 `live_order`／`live_fill`／`live_pending_action` 皆為 0 筆**——下單路徑一次都還沒被執行過，驗收 #1 的「每筆委託、成交」與 #4 的 parity （零委託時恆為 `UNEXPLAINED = 0`）都還沒真正驗到。9/29 是第一個可能送出委託的日子。2026-09-22 演練前準備完成並以 launchd 排程啟動（`scripts/launchd/rehearsal_schedule.py`），演練交易日 9/23、9/24、9/29、9/30、10/1。相依 Phase4-6~Phase4-8、Phase5-5；**1S 另相依 Phase6-1，1F 另相依 Phase6-2、Phase6-5、Phase6-6** |
| Phase7-2S | 正式環境小額上線檢查表（股票） | `docs/live/production-checklist-stock.md` | 使用者逐項勾選 | ⬜ | **股票線**｜相依 Phase0-1（已完成）、Phase7-1 的 1S。**無硬日期**——期貨線的 Phase6-6 卡 2026-10-20，本步驟不受它影響 |
| Phase7-2F | 正式環境小額上線檢查表（期貨） | `docs/live/production-checklist-futures.md` | 使用者逐項勾選 | ⬜ | **期貨線**｜相依 Phase0-1（已完成）、Phase7-1 的 1F，以及 Phase6-6 於 2026-10-20 換月日實跑通過 |
| Phase7-3 | 舊 Shioaji 工具收斂                                          | `core/utils/order.py`、`core/utils/callback.py`、`core/utils/account.py`、`core/pipeline/tw/updaters/*_tick_updater.py`、`scripts/manual/manual_tick_crawler.py`                          | `grep -rn "OrderUtils\|Callback.order_callback"` 無結果；tick updater 冒煙通過                                                                                         | ✅ | 2026-09-22 完成：`OrderUtils`、`Callback`、`ShioajiAccount` 刪除，tick updater 與手動爬蟲改用 `ShioajiSession`（多帳號唯讀登入）；grep 無殘留、2203 passed、tick 冒煙 2330 單日 4,824 筆。相依 Phase2-2 |
...
| Phase7-5 | 並行段落互相標記 CRASHED，並靜默解除帳戶層 halt | `core/dao/tw/live_trade_dao.py` 的 `mark_crashed_runs()`、`core/live/trader.py`、`core/live/after_close.py` | 兩個段落時窗重疊時，後啟動者不得標記前者為 CRASHED；前次 `REDUCE_ONLY` 必須讀得回來 | ✅ | 2026-09-24 完成。**不只重疊會觸發——任何一次崩潰都會**，修法比原規劃收斂 |
| Phase7-6 | 日線報價的 `date` 在實盤與回測型別不同 | `core/broker/tw/shioaji_quote_stream.py`、`core/live/datafeed/tw/futures_live_datafeed.py` | 8 條新測試；回歸雙線逐筆相同 | ✅ | **2026-09-24 演練尾盤段的實際崩潰**，策略被降級為 HALTED、當日零委託。相依：無 |
...
```
入口層        run.py（--mode backtest | live）  ← 入口拆分後為 apps/backtest.py 與 apps/live.py
                │
...
└── live/
    ...
    ├── segment.py                      # 段落定義（open／close／after_close／intraday）
    ├── after_close.py                  # 盤後：報表、成本校正、parity
    ├── account_sync.py
    ├── reconciler.py
    ├── strategy_guard.py               # 策略層的啟動與降級守門
    ├── termination.py                  # LiveTerminated（SIGTERM 收尾，繼承 BaseException）
    ├── capital_allocator.py              # 多策略資金額度保留／釋放
    ├── attribution/                      # 多策略歸屬
    │   ├── position_ledger.py            # live_position_lot 讀寫、策略 ⇄ 部位
    │   ├── conflict_guard.py             # D8 同標的守門
    │   └── resync.py                     # --resync-from-broker：以券商部位重建歸屬帳
    ├── oms/order_manager.py
    ├── risk/{risk_config.py, risk_manager.py, trading_mode.py, margin_gate.py, event_log.py}
    ├── datafeed/{base.py, calendar.py, tw/stock_live_datafeed.py, tw/futures_live_datafeed.py}
    ├── intraday/{event_loop.py, session_guard.py}
    ├── notify/{base.py, telegram_notifier.py, factory.py}
    └── report/{live_reporter.py, parity_checker.py}
```

`core/broker/` 另有 `execution_dedup.py`（回報去重）與 `tw/quote_replay.py`（錄製重放）。

**`intraday/` 沒有 `bar_builder.py`**，這是 D2 的裁示（逐筆觸發，不自建 K 棒）——
盤中策略直接吃 tick，要聚合就自己在策略內做。
...
- **帳戶層的組合績效在回測端無法重現**（2026-09-19 補，本節最大的一條）：D0 保證了「單支策略」
  ...
  **不能相加**當成帳戶預期績效，而 parity 的 `CROSS_STRATEGY_BLOCKED` 與 `CAPITAL_EXHAUSTED`
  ...
  「判定」與「讀寫」分開（判定放 `core/portfolio/aggregation.py`，讀寫留在 `core/live/`），
...
> **實連紀錄（2026-09-21 台北時間，模擬環境）**
>
> 條款簽署、股票 API 權限、模擬環境登入皆完成。當時 shioaji `1.3.3`。
>
> **1.7.5 重驗（2026-09-22 盤中，模擬環境）**：股票與期貨帳號都已取得（下方「期貨 API 權限未開通」已解除），股票與期貨各送一筆 ROD 限價單（跌停價，不成交）並撤單，`update_status()` 分帳號刷新後都查得到 `Cancelled`。合約欄位改變：`update_date` 由斜線字串改為 `datetime.date`、`unit` 改為 `float`、股票不再有 `multiplier`／`underlying_code`；期貨分類為 373 類（Shioaji 升級至 1.7 的重驗與實連驗證，已完成並移出）。
>
...
- **做法**：在 `scripts/check_layer_deps.py` 做以下調整：
  - `_LAYER_RULES` 新增：
    ...
    - `("core.portfolio", 4, "部位建構層", False)`：**已登記完成，本步驟不必再加**（隨 `core/portfolio/` 建立時一併登記）。實際白名單比原先規劃寬：可 import `core.models`／`core.utils`／`core.config`／`core.api`，**不可 import `core.backtest`／`core.live`／`core.strategies`**；`FuturesPortfolioConstructor` 需要的 `FuturesMarginConfig` 取自市場結構層（`core/market/tw/futures_margin_config.py`），比本層低，屬正常向下相依而非同層邊。跨策略仲裁的判定（`aggregation.py`）也在這一層。
  ...
  - **預期會出現的「同層互相 import」**（腳本只列出、不影響結束碼，但要在本步驟先登記理由，避免之後被當成新問題）：
    - `core.live.datafeed` → `core.datafeed.base`：`LiveDataFeed` 繼承 `BaseDataFeed`，因為策略的 `setup_apis(feed)` 型別就是它。該契約已於 2026-09-22 搬到中立的 `core/datafeed/base.py`，兩邊同層。
...
- **做法**：`core/utils/constant/` 依 §2.7 先定義模組常數，再定義 Enum：
...
- **產出**：`core/utils/constant/`、`core/utils/__init__.py`。
...
>     「券商 Enum 缺成員時必須拋出、不可退回」的保證改用缺成員的替身 Enum 驗證，不再依賴安裝版本（Shioaji 升級至 1.7 時修正，已完成並移出）。
...
> 股期對照表的分類改讀合約的 `root`（Shioaji 升級至 1.7 時改寫，已完成並移出）。
>
...
> - 逐筆：訂閱 2330 後收得到 `tick_stk` 並轉成報價；2026-09-22 錄製的 28 筆逐筆行情重放兩次逐筆相同（Shioaji 升級至 1.7 的模擬環境實連驗證，已完成並移出）。
...
- **做法**：
  ...
  - **本庫維持 SQLite、不隨 [PostgreSQL遷移計畫.md](PostgreSQL遷移計畫.md) 遷移**（2026-09-25 定案）：實盤行程是唯一寫入者，用不到網路資料庫的並發寫入；「先寫 DB 再送單」搬到網路資料庫後，資料庫斷線就不能送單，平白多一個故障點。
...
> 接管後的單也撤不掉。已由專案優化與改善清單（已完成並移出）修正（壓縮碼只由 OMS 產生一次，broker 直接使用）。
>
...
>
> **✅ 官方開休市日曆完成（2026-09-22）**
> - **資料來源**（2026-09-22 實測）：`https://www.twse.com.tw/rwd/zh/holidaySchedule/holidaySchedule?response=json&date=YYYY0101`，一年一次請求。清單混著休市日與提醒列：名稱含「開始交易」「最後交易」「補行交易」的是提醒（當天照常交易），其餘一律休市，**含「市場無交易，僅辦理結算交割作業」**。2026 年近期的休市日：9/25（中秋）、9/28（教師節）、10/9（國慶日）。期貨沿用同一份（期貨市場休市日原則上與證券市場相同）。
> - **ETL**：`MarketHolidayCrawler`／`Cleaner`／`Loader`／`Updater`、`MarketHolidayDAO`、`MarketHolidayAPI`，表 `tw_stock.db` 的 `market_holiday`（`date` PK、`name`、`description`、`is_trading_day`、`year`）。重跑同一年以 savepoint「先刪整年再寫入」；更新去年、今年、明年三個年度，明年尚未公告（`NO_DATA`）時略過，抓取失敗的年度略過但 target 記為失敗（結束碼 1）——實盤靠這張表判交易日，失敗不可靜默。`DataType.MARKET_HOLIDAY` 自動落在預設的 `no_tick` 與 `all` 內。
> - **實盤接線**：`OfficialHolidayCalendarSource` 排在股票與期貨資料源預設來源清單的第一個；**只替已入庫的年度作答**（未入庫回 None，不把「表上查不到」當成「不是假日」）。期貨資料源另開一條 `tw_stock.db` 的唯讀連線（新參數 `stock_db_path`）——**只有期貨資料、沒有 `tw_stock.db` 的機器，期貨資料源啟動會失敗**，刻意不吞掉。
> - **衝突語意不變**：休市日若券商合約檔剛好更新，官方說休市、合約檔說開市，`resolve_trading_day()` 會拒絕啟動（結束碼 3）。以前只剩合約檔一個來源時，這種日子會被判成開市而進入送單路徑；現在最壞是休市日的排程以結束碼 3 結束，排程告警要能接受這一點。
> - **換月日曆**：`inject_roll_calendar()` 的未來交易日改為「平日扣掉官方休市日」，Phase6-6 記的「未來國定假日排除不了」在已入庫年度內已解除；未入庫年度（12 月公告前的明年）仍以平日近似。
> - 颱風等臨時停市不在公告表上，這張表無法反映（寫進資料說明與部署文件）。
> - 測試：`tests/test_market_holiday.py`（分類規則、清洗、入庫冪等、API 三值語意、updater 略過未公告年度與失敗記錄、預設 target 集合）、`tests/live/test_official_holiday_calendar.py`（兩個資料源接上官方來源、衝突時拒絕、換月日曆排除 9/25、9/28、10/9）；fixture 為 2026-09-22 實抓的 2026 年回應。反向驗證：分類只看「交易」字樣、資料源拿掉官方來源、換月日曆不扣休市日，各有測試變紅。
> - 驗證：`uv run pytest` 2200 passed；ruff、三支檢查腳本、`pre-commit run --all-files` 全綠。**未寫正式資料庫**：正式的 `tw_stock.db` 還沒有 `market_holiday` 表，要先跑 `python -m tasks.update_db --target market_holiday`（之後每天的預設 `no_tick` 更新會順便帶到）；在那之前官方來源回 None，行為與今天之前相同。
...
- **做法**：
  - 盤後（Phase4-6 之後）以當日正式資料對同一支策略跑一天回測（`start_date = end_date = 當日`），取出「回測會送出的委託清單」。
    - **輸出一律寫到 `LIVE_RESULT_DIR_PATH` 底下，不可寫進 `RESULTS_DIR_PATH/<策略名>/`**：那裡是回歸雙線的比對基準，被每日 parity 跑出來的單日結果覆蓋掉，`run_regression.sh` 就失去意義了。
      > **2026-09-21 更正**：實作沒有照做——單日回測照常寫進 `RESULTS_DIR_PATH/<策略名>/` 並掛上 backtest logger。上面的理由也不準確：回歸比對不經過 reporter，被破壞的是研究者的完整回測報表與 log（實盤行程的 log 也會一併寫進 `logs/backtest/`）。已由專案優化與改善清單（已完成並移出）修正：parity 的單日回測以 `write_artifacts=False` 建立，不寫任何產物。
...
> **🔄 進度紀錄（2026-09-22）：程式完成，期貨已以真實交易驗證，剩股票實跑**
> - **模型與查詢**：新增 `RealizedTradeSnapshot`（`core/models`，給不出的欄位留 None）；`ShioajiAccountQuery.get_realized_trades(date)` 分別查股票與期貨帳號的 `list_profit_loss()` 並正規化（期貨代碼換成 `{商品}{YYYYMM}`），`ShioajiBroker.get_realized_trades()` 委派。
> - **比對**（`AfterCloseRunner.calibrate_costs()`，取代原本依不存在介面寫的 `backfill_actual_costs()`）：以「一筆已平倉交易」為單位，實際成本期貨取 `fee + tax`、股票以 `live_fill` 回查開倉成交（新增 `LiveTradeDAO.get_fills_by_seqno()`，不限日期）算毛損益再減券商淨損益；估算值由 factory 的 `make_trade_cost_estimator()` 依市場重算（與實盤部位管理同一組預設成本設定）。結果寫 `<實盤報表目錄>/account/<日期>_cost_calibration.csv`，差距超過 20% 寫 `COST_MODEL_DRIFT` 事件；算不出來的列照樣寫出並註明原因，不略過。
> - **實測抓到的口徑**：第一版照「來回」估期貨（兩次手續費＋兩腿期交稅），以當天模擬帳戶的真實交易驗證時估算剛好是券商的兩倍（486 對 243）——券商已實現紀錄的 `fee=50`、`tax=193` 是**單邊**。已改為只估平倉那一腿，驗證後估算與券商扣款一致（243 對 243），並以測試釘住。
> - 股票的估算：手續費開平倉各一次；證交稅只課賣出那一腿（多單課在平倉價、空單課在開倉價），開倉日等於交易日時用當沖稅率。
> - `LiveTradeDAO.backfill_fill_costs()` 與 `live_fill.actual_*` 欄位保留（日後券商提供逐筆費用時可用）；**`live_fill.estimated_*` 從未被寫入**（OMS 寫成交時沒有帶），報表的估算費用欄目前是空的——本步驟以交易為單位比對，不依賴它，但要讓成交明細帶估算值需另外處理。
> - 測試：新增 `tests/live/test_cost_calibration.py` 11 條（查詢正規化、股票多空稅別、當沖稅率只限同日、期貨單邊、缺資料回 None、股票實際成本＝毛損益減淨損益、期貨實際成本、偏差事件與報表、缺開倉成交照列並註明、閘道不提供時略過）；拿掉多空方向、多單稅課錯腿、不寫偏差事件，各有測試變紅。
> - **剩下**：股票那一側要本地有開倉成交才比得到（今天的模擬成交是測試直接呼叫券商送的，沒有經過 OMS 寫進 `live_fill`），Phase7-1 演練的第一個盤後就會產生，屆時確認股票的估算偏差在門檻內。
...
> - **寫這條測試時先寫出一個假綠燈**：第一版沒設 `live_schedule` 也沒走 `prepare()`，
>   兩次跑出來都是空清單卻「通過」。已補 `assert first` 先確認真的有訊號——
>   比對「兩次結果一致」的測試，必須先證明那個結果不是空的。
...
> - **`to_tick_quote()` 的欄位全部取自 2026-09-21 實錄**，並撈到兩個規劃文件沒有的陷阱：
>   `simtrade`（試撮，是假成交，當真會讓策略對著不存在的量做決策）與
>   `intraday_odd`（盤中零股，`volume` 單位是股不是張，**差 1000 倍**）。
> - **心跳由 `queue.get(timeout=)` 逾時產生，不另開執行緒**：多一條執行緒就多一份
>   要與事件迴圈同步的狀態，而心跳要做的事本來就只是「沒有事件時也要醒一次」。
...
> 已由專案優化與改善清單（已完成並移出）補上（心跳先 `ensure_connected()` 再跑日終動作）。
...
> **✅ 接線完成（2026-09-21）**：與 Phase5-2 同一次施作，完整紀錄見該步驟——行情與回報導進同一個事件 queue、`apply_event()` 逐事件消化、`run()` 依 `timing` 分派。本步驟的部分是心跳呼叫 `ensure_connected()`，以及行情中斷降 `REDUCE_ONLY` 而不降 `HALTED`。
### Phase5-4. 日終強制動作 ✅
...
>
> **✅ 結案（2026-09-22）**：期貨 API 權限已開通（Phase0-1 的 1.7.5 重驗），但實盤的期貨段落時窗只有日盤（`TW_FUTURES_SEGMENTS`：08:45～13:50），部署文件的排程也沒有夜盤段——**實盤行程在夜盤時段不會運作，夜盤成交不會發生**，「帳務日與行情查詢日混用」在實盤沒有發生的路徑。**偏離原規格**：`resolve_accounting_date()`／`resolve_quote_date()` 已寫好並有測試，但不接到 `account_sync` 與 `futures_live_datafeed`——沒有夜盤成交時接上去等於沒有測試能驗的死路徑。**日後新增夜盤段落（或盤中模式跑到夜盤）時，必須先把這兩個函式接上成交歸屬與行情查詢**，並以 23:59→00:01 與星期五夜盤兩個情境驗證。
...
> **🔄 進度紀錄（2026-09-22）：送單前資格檢查完成，剩模擬環境實送**
> - **實測值域（模擬環境，唯讀）**：`api.short_stock_sources([contract])` 回每檔 `short_stock_source`（可融券數，以張計；當天 2317 為 984、2330 與 0050 為 0）；合約 `day_trade` 是 `DayTrade` Enum，值域 `Yes`／`OnlyBuy`／`No`。
> - `ShioajiBroker.place_order()` 在送出前呼叫 `_check_stock_eligibility()`，不合格時**本地回 REJECTED 並附原因、不送出、不佔下單額度**（OMS 本來就接受 `PENDING_SUBMIT → REJECTED`）：
>   - 先賣後買的當沖（`ShortMethod.DAY_TRADE` 的賣出）：合約 `day_trade` 要是 `Yes`；
>   - 先買後賣的當沖（`is_day_trade` 的多單買進）：`Yes` 或 `OnlyBuy`；
>   - 融券賣出（`ShortMethod.MARGIN` 的賣出）：券源不少於委託張數；**查不到券源（查詢失敗或回傳沒有這檔）一律不送**，對應回測的 `rejected_no_borrow`。
>   - 欄位值認不得時照「不允許」處理；Enum 依值比對。借券（`SBLShort`）的額度由券商議借，不在這裡檢查。
> - **偏離原規格（產出位置）**：規格寫 `shioaji_order_mapper.py`、`risk_manager.py`。當沖與券源都是券商端的即時資訊，要連線查詢，放 mapper（純轉換）或風控（市場無關、不連券商）都不對，故放在 `ShioajiBroker` 送出前。
> - **融資做多（D3）**：mapper 對多單一律送 `Cash`，從不產生 `MarginTrading`，第一階段不開放融資已成立，不需另外擋。
> - 測試：新增 `tests/live/test_stock_order_eligibility.py` 15 條（先賣後買四種合約值、先買後賣三種、Enum 依值、一般現股不檢查不查券源、券源足夠／不足四種、查不到券源擋單、融券回補不查券源）；拿掉檢查呼叫時 7 條變紅，查不到券源改成放行時對應那條變紅。
> - **剩下**：下一個交易日盤中執行 `uv run pytest tests/live/test_shioaji_sim.py -m shioaji_sim_order -k short_sell -v -s -p no:randomly`——以漲停價各送一筆 2317 融券賣出、借券賣出、當沖先賣並撤單，確認券商收得下三種委託條件（借券是第一次實送）。

> **待評估：以券商的券賣資格旗標取代「用券源張數推資格」**（原為 shioaji 1.7 新 API 的評估結論，隨該清單移出併入本步驟）
> - `api.contracts.info(contract)` 回的 `StockInfo` 有 `sbl_shortable`／`margin_shortable`／`below_ref_shortable` 與 `trading_suspended`／`disposition_level`，現行解析器用的 `api.Contracts` 拿不到。現行送單前檢查只看 `day_trade` 與 `short_stock_sources()` 的券源張數，「這檔可不可以融券／借券」是由券源有沒有量推出來的；旗標是券商直接給的答案，且能多擋處置股與暫停交易。
> - 2026-09-22 模擬環境實測 2330：三個旗標皆 `True`、`disposition_level=0`、`trading_suspended=False`，但券源為 0——正是「有資格但查不到券源」的情況，現行邏輯會一律不送。
> - **不必整個遷到 `api.contracts`**：`api.Contracts` 在 1.7.5 沒有棄用警告（`warnings.simplefilter("always")` 實測），換過去拿到的是同一批資訊；期貨那側 `futures("TXF")` 照樣含 `TXFR1`／`TXFR2` 連續月別名，排除邏輯省不掉。兩套可混用：`api.snapshots()`、`api.short_stock_sources()` 吃 `BaseContract` 與 `StockInfo` 都可以。
> - 等本步驟模擬環境實送的結果再決定是否採用。
...
> **⏸ 暫緩（2026-09-22）**
> - **為什麼**：規格只寫了「乘數交叉比對」，但實盤的股期**連送單路徑都沒有**——`ShioajiBroker._build_order()` 對 `FuturesOrder` 一律走 `resolve_index_futures()`，股期商品不在 `SHIOAJI_FUTURES_CATEGORY` 對照表內，下單會直接拋 `LookupError`。目前也沒有任何策略交易股期（[暫緩工作彙整.md](暫緩工作彙整.md) S4 同樣在等股期策略）。沒有路徑也沒有使用者時，單做交叉比對驗不到任何東西。
> - **已知事實（2026-09-22 模擬環境實查，解除時不必重查）**：
>   - `resolve_stock_futures("2330", "202610")` 取得 `CDFJ6`，`underlying_code` 2330、`multiplier` 2000.0（`float`）；股期對照表 281 檔，快照正常。
>   - **68 檔標的同時有標準型與小型股期**（含 2330、2454、0050），對照表取先掃到的那個——**要下哪一種必須由呼叫端明確指定分類**，否則乘數可能差 20 倍。
>   - 成交回報與部位的代號轉換（Phase6-2）對股期同樣生效，但分類碼不在商品對照表內，會原樣保留成 `CDF202610` 這種形式；股期的 `FuturesOrder.product` 要約定成分類碼才對得起來。
>   - 保證金檢查（Phase6-5）的需求計算走 `FuturesPositionManager.calculate_margin()`，股期會落到比例表（`stock_futures_margin_rate_history`）；成本校正（Phase4-9）的估算器對股期回 None（乘數不在常數表）。
> - **解除條件**：開始開發股期策略時。屆時要一起做：股期的送單路徑（依明確的分類解析合約）、乘數以合約 `multiplier` 與 `futures_stock_universe` 交叉比對（不一致就拒絕交易並寫事件）、`product` 的代號約定、成本估算器支援股期乘數，並照原規格在模擬環境實查標準型、小型、ETF 期貨各一檔。
...
> **⏸ 暫緩（2026-09-22）**
> - **做了什麼**：`StockOrder` 建構時檢查 `order_lot=IntradayOdd` 的 `volume` 必須是 1～999（超出就拋，不等到送單）；`ShioajiBroker` 送出前**拒絕盤中零股單**（本地回 REJECTED 並附原因）。mapper 原本就擋「零股超過 999 股」。測試在 `tests/live/test_stock_order_eligibility.py`（建構 3 條、送單前拒絕 1 條）。
> - **為什麼不照規格開放**：規格只要求「單位不能混淆」的檢查，但真正會出事的是下游——委託金額換算（`spec.to_units()`）、成本模型、`StockPositionManager`、歸屬帳 `live_position_lot`、以券商部位對帳（`list_positions()` 以張回傳），全部以「張」為單位。只開放送單的話，一張 500 股的零股單成交後會在各處被當成 500 張，差 1000 倍。回測也不支援零股（規格已提到要在策略 docstring 註明差異），目前沒有任何策略會產生零股單。
> - **解除條件**：有策略需要零股時。屆時要一起處理：委託金額與成本以股計、部位管理與歸屬帳的單位、零股部位的對帳（`list_positions(unit=Share)`），並拿掉 `ShioajiBroker` 的拒絕與本段說明；驗證沿用原規格（錯配單元測試、模擬環境送一筆零股）。
...
> - **實盤換月日曆**：`TwFuturesLiveDataFeed.inject_roll_calendar()` 以資料庫的實際交易日（往回 400 天）加上未來平日（往後 70 天）建 `FuturesCalendar`，注入換月設定。**已知限制**：未來的國定假日排除不了，距到期日之間夾著假日時會多算一天、換月可能晚一天（例如最後交易日前一天恰為假日時，會拖到最後交易日當天，而那天尾盤段舊月已收盤 → 平倉腿失敗、換月放棄、舊月由交易所結算）。要消除得等官方休市日曆（Phase4-2 已知限制）。（**2026-09-22 已改善**：官方開休市日曆完成後，已入庫年度的未來休市日會被扣掉，見 Phase4-2）
...
> **待評估：以券商的 `last_trading_date` 交叉驗證換月時點**（原為 shioaji 1.7 新 API 的評估結論，隨該清單移出併入本步驟）
> - `api.contracts` 的 `FuturesInfo` 有 `last_trading_date`，現行 `api.Contracts` 拿不到；換月時點目前由專案日曆推算最後交易日，券商這個欄位可當交叉驗證。實測 `TXFJ6` → `2026-10-21`，與專案推算一致。
...
> **演練前準備（2026-09-22 完成）**：兩支演練策略原本跑不起來，而且不會報錯——
> - `MomentumStrategy1`、`MomentumFuturesStrategy` 補上 `live_ready`、`live_schedule`（開平倉鉤子都在尾盤段）與 class docstring〈實盤執行〉區塊，通過 `inspect_strategy()`。
> - **實盤只抓 `context.symbols` 的報價，而兩支策略都沒宣告標的池**：資料源在空清單時直接回空，演練會整天沒有任何訊號。新增 `TwStockLiveDataFeed.fill_default_universe()`（沒宣告時以前一交易日價格表上的每一檔為標的，與回測每天的標的池相同）與 `TwFuturesLiveDataFeed.fill_default_contracts()`（`products` 目前掛牌的各月份）；factory 改為在資料源 setup 之後才讀 `context.symbols`（原本建 context 時就複製了空清單）。
> - **`MomentumStrategy1` 的「前一交易日」在實盤會靜默算錯**：它查依回測區間（2020-05～2025-05）預建的清單，實盤日期超出清單時平移會落到清單最後一天，拿一年多前的收盤當昨收。改為日期超出清單時回頭查資料庫；回歸雙線通過（回測日期都在清單內）。
> - 測試：`tests/live/test_rehearsal_readiness.py` 8 條；拿掉範圍防護、factory 不重讀，各有測試變紅。
> - **排程（2026-09-22 安裝）**：`scripts/launchd/rehearsal_schedule.py --install` 在本機 `~/Library/LaunchAgents/` 裝了 9 個 `com.alphaedge.live.*` 排程——`update_db`、股票與期貨各自的 open／close／after_close、每 5 分鐘的存活監控（限台北 08:00～15:00），以及 9/23 09:30 一次性的放空實測（Phase6-1）。以 `uv` 直接執行、不經容器；本機美東時區，台北快 12 小時，由腳本換算並錯開星期。安裝後以臨時排程驗證 launchd 環境能讀到 `.env` 並登入模擬環境。**演練結束後以 `--uninstall` 移除**；美東 11/1 改回標準時間，若屆時仍在用要重新安裝。
> - **演練注意**：模擬帳戶原本就有 6 檔股票部位（2362、2425、3149、4956 多單，6134、6151 空單，不是本系統開的），第一次啟動會被收進 `__unattributed__`（只允許平倉）。演練期間交易日為 9/23、9/24、9/29、9/30、10/1（9/25、9/28 休市，正好涵蓋「跨休市日」的要求）。

> **⛔ 演練第 1 天（2026-09-23）：三個段落全部失敗，當日作廢**
>
> `stock-open`、`futures-open`、`sim-short-sell` 結束碼皆為 1，**同一個原因**，
> 全部停在 `prepare()`，後面什麼都沒執行：零委託、零成交、沒跑對帳、
> 沒有 parity 可比、成本校正拿不到股票側資料。
>
> ```
> ValueError: Σ 各策略額度 1,000,000 超過帳戶總權益 0 × 95% = 0，
>             缺口 1,000,000（MomentumStrategy1=1,000,000）
> ```
>
> **根因鏈（已逐層追到底）**：
>
> | 層 | 值 | 原因 |
> |---|---|---|
> | `LiveTrader._account_equity()` | 0 | ＝ `allocator.available_balance + Σ allocator.used` |
> | `available_balance` | **0** | `broker.get_account().available_balance`；模擬環境的股票帳務欄位回 0 |
> | `Σ used` | **0** | `{策略名: position_value(策略帳戶)}`；帳上那 6 檔屬 `__unattributed__`，**不屬於任何策略**，故兩支策略的持倉占用都是 0 |
>
> **啟動守門本身是對的**：資料新鮮度（最新日 2026-09-22）與交易日判定都通過，
> 擋下來的是額度檢查，而它擋得有道理——是**比較基準**被算成 0。
> `_refresh_capital()` 的呼叫順序也正確（在 `verify_quota()` 之前），不是順序問題。
>
> **兩個獨立缺口，今天一起發作**：
>
> 1. **模擬環境的股票帳戶可用餘額回 0**。這與已知的「期貨保證金查詢所有欄位一律為 0」
>    是同一族問題——**期貨那條已經做了兩層檢查繞過，股票這條沒有**，於是把整個演練擋住。
> 2. **`__unattributed__` 的部位不計入 `Σ used`**。即使券商餘額正常回報，
>    只要帳上有無歸屬部位，比較基準就會少算它們的價值，檢查比應有的更嚴格。
>
> **這不是偶發**：今日剩餘的收盤段、盤後段，以及之後每一天的每一次啟動，
> 都會以完全相同的原因失敗。修掉之前演練無法推進。
>
> ---
>
> **✅ 順帶證實：Shioaji 模擬環境「保留」隔夜部位。**
>
> 先前對「模擬環境可能每晚清倉」的疑慮**不成立**。9/23 08:30 的啟動 log 顯示
> 券商端仍有 6 檔部位，且全部「在歸屬帳中查無來源」。
> 對照 2026-09-22 的實測紀錄：代號（2362、2425、3149、4956 多、6134、6151 空）
> 逐檔相同，且 `Σ 數量 × 價格 = 514.89`——與當日記錄的那個數字**一字不差**。
> 同一批部位跨過了 9/22 → 9/23 的日界。
>
> 因此**多日演練的隔夜驗收項目有效**，不需要改設計或延長天數。
> 但「**我們自己開的**部位會不會留過夜」仍未驗證——那要先送得出委託才談得上。
>
> ---
>
> **其他觀察**：
>
> - **`update_db` 跑了 3 小時 37 分**（台北 08:00 → 11:37），
>   **遠遠超出「08:30 開盤前跑完」的 30 分鐘視窗**。耗時主要在權益變動表的逐檔回補。
>   今日 08:30 的新鮮度檢查之所以通過，是因為 `price` 早已更新到 2026-09-22，
>   與那次 `update_db` 無關——**這個前提從一開始就不成立，只是今天沒被它擋到**。
> - **`update_db` 結束碼非 0：`finmind` target 失敗**，其餘 14 個 target
>   （chip、corporate_action、dividend、fs、futures_chip、futures_continuous、
>   futures_margin、futures_price、futures_stock_universe、margin、market_holiday、
>   mrr、no_tick、price）全部成功。
>   原因是**已知的帳號等級限制**，非本次重構造成：
>   `Your level is register. Please update your user level.`
>   ——`register` 等級沒有券商分點資料集權限。
>   **它在第 1／2,647,134 個組合就中止並大聲失敗**，沒有浪費時間，
>   也沒有像以前那樣被吞成「無資料」。
> - `price` 本批 243 requested / 2 ok / **241 unreachable**——即進度檔裡既有的
>   241 個歷史缺口，非本次新增。
> - 三支日頻 updater 的 template method 與財報 updater 拆分後的
>   `update_statement(spec)`、`equity_change` 子模組、改名後的
>   `throttle_equity_change()` **都在真實 ETL 上實際執行且行為正確**
>   （log 可見三張報表的啟動訊息逐字正確、節流秒數正確）。
>
> **✅ 演練第 2 天（2026-09-24 台北）：Day-1 的四個修正全部端到端驗證通過**
>
> 兩天同一段落的日誌併排在同一個檔案裡，對照如下：
>
> | | Day-1 | Day-2 |
> |---|---|---|
> | `stock-open` 額度檢查 | `ValueError: Σ 額度 1,000,000 超過總權益 **0** × 95%` → 崩潰 | `資金額度檢查通過：Σ 額度 400,000 ≤ 總權益 **582,608** × 95%` |
> | `stock-open` 對帳 | `對帳不一致：本地兩份紀錄 6 筆不一致` | `對帳一致` |
> | `futures-open` 額度檢查 | `ValueError: Σ 額度 3,000,000 超過總權益 0 × 95%` → 崩潰 | `模擬環境查不到帳務（欄位整組回 0），略過額度總量檢查`（WARNING） |
> | `futures-open` 對帳 | 同樣沒跑到 | `對帳一致` |
>
> 四項修正（總權益改取券商 `total_equity`、帳戶依商品分派、模擬環境查不到帳務時
> 跳過額度檢查、`__unattributed__` 不計入內部一致性）**全部在真實模擬環境生效**。
>
> Day-2 不再出現六行「查無來源，已收進 `__unattributed__`」，只有一行
> `由歸屬帳重建本地部位：{'__unattributed__': 6}`——歸屬帳的 lot 跨行程存活且被認得。
>
> ---
>
> **⚠️ 但 Day-2 撈出一個每天都會發作的新缺陷：並行段落互相標記為 CRASHED，
> 且會靜默解除帳戶層 halt。** 已另立 **Phase7-5**，Day-2 之後的觀察要以它為準。
>
> **它是被前述修正「成功」才暴露出來的**：Day-1 的 `stock-open` 在啟動後 4 秒就
> 崩潰、`ended_at` 立刻寫上，與 `futures-open` 沒有重疊；修好之後 `stock-open`
> 活到收線（台北 09:05），而 `futures-open` 台北 08:40 啟動——重疊 25 分鐘。
>
> **另外兩點在查證 `--resume-trading` 用法時發現，一併記下：**
> - **`AfterCloseRunner` 只呼叫 `mode_state.load()`，從不套用 resume。**
>   `--phase after_close --resume-trading` 會**靜默無效**：旗標被接受、
>   halt 沒解除，而使用者會以為解除了。（`LiveTrader.prepare()` 的順序是對的：
>   `load()` → `resume()`。）
> - `_wait_until_submit_window()` 只在「還沒到」時等待；**時窗已過時直接返回並繼續送單**。
>   所以不能靠「挑一個時窗已過的段落」來規避送單。
>
> **兩段收線時的競態實錄（台北 09:05）**——halt 以 **195 毫秒**之差存活：
>
> ```
> futures-open  ended 09:05:00.826  NORMAL
> stock-open    ended 09:05:01.021  REDUCE_ONLY   ← 晚 195ms，於是它算數
> ```
>
> 另注意：`stock-open` 自己的 `finish_run()` 把先前被誤標的 `CRASHED`
> 覆寫成 `帳戶層交易模式 REDUCE_ONLY`——**誤標記會自我抹除**，
> 事後 `live_run` 上看不出曾經發生過，只剩當下那則 CRITICAL 告警。
> 這讓 Phase7-5 的缺陷更難被發現。
>
> **人工解除降級（台北 09:05:50）**：降級成因是已修掉的對帳 bug、當日對帳已一致，
> 故以 `--phase open --dry-run --resume-trading` 明確解除，讓狀態不再由競態決定。
> 驗證三點皆通過：日誌出現「帳戶層交易模式由人工恢復為 NORMAL」；
> `live_run` 最後一筆已結束紀錄的 `account_mode` 為 `NORMAL`；
> `live_strategy_mode` 記下 `MomentumStrategy1 / NORMAL / 人工恢復`；
> 當日 `live_order` 零列（沒有送出任何委託，dry-run 也沒產生 ticket）。
>
> **⚠️ 一併更正本紀錄稍早的推論**：開盤段的「本段落沒有任何委託」
> **不能歸因於 `REDUCE_ONLY`**。`MomentumStrategy1` 的開倉與平倉鉤子
> **都宣告在 `AT_CLOSE`**（見演練前準備），所以它在開盤段本來就不會出訊號——
> 解除降級後以 `NORMAL` 重跑一次同樣是零委託，可為佐證。
> **真正會下單的是尾盤段**，Day-2 的交易驗證要看台北 13:20 那一段。
>
> ---
>
> **尾盤段（台北 13:20）：第一次真正跑到策略鉤子，就掛在那裡。**
> 四道啟動關卡全過（`Σ 額度 400,000 ≤ 總權益 584,350 × 95%`、`對帳一致`、
> 模式 `NORMAL`、結束碼 0），然後 `MomentumStrategy1` 的鉤子拋出
> `can't compare datetime.datetime to datetime.date`，策略被降為 **HALTED**，
> 當日零委託。根因與修法見 Phase7-6。
>
> 期貨尾盤段同樣結束碼 0、對帳一致，零委託是**策略判斷的結果**（沒出訊號），
> 不是缺陷。
>
> **Phase7-5 的缺陷如預期再次發作**：`futures-close` 台北 13:28 啟動時把還在跑的
> `stock-close` 標成 CRASHED 並推 CRITICAL；`stock-close` 13:35 收線時
> 自己覆寫成「正常結束」，痕跡再次抹除。連續兩天發作、兩天都自我抹除。
>
> **盤後段（台北 14:30／14:35）：完整跑完，Day-1 從未到達的路徑。**
> `對帳一致`、`成本校正 0 筆`、9 個報表檔案輸出
> （`__account__`／`__broker__`／`__unattributed__` 各 3 份，
> 部位表各 6 列＝接管的隔夜部位）、
> **`訊號 parity 比對通過：無未解釋的差異`（`UNEXPLAINED = 0`，對應驗收標準 4）**。
> 即使當日零委託，這一段仍驗到了對帳、成本校正、報表與 parity 四條路徑，
> 並證明「零委託時 parity 不會誤報」。
>
> **一個正面驗證**：`[Quote] 020011 快照無有效成交價，略過該檔`——
> 報價驗證層在實盤第一次發揮作用，券商快照缺價時略過該檔，
> 而不是產出一個 0 元報價送進策略。
>
> **Day-2 收尾（台北 14:47）**：兩個缺陷修完後解除 `MomentumStrategy1` 的 HALTED，
> 並實跑確認策略已能算出漲幅（6925／6933／7547 各 10.0% 等 6 檔達標）。
> 9/29 起的演練日應該能真正送出委託。

> **✅ 休市日驗證（2026-09-25 台北，中秋節）：六個段落全部正常結束**
>
> 這一天不是演練交易日，但排程照跑——**這正好是規劃裡「演練期間至少跨一個休市日」
> 那條驗收**：官方開休市日曆答 `False`，六個段落沒有一個誤判成開市。
>
> 紀錄庫 `live_run` 實查：
>
> | run_id | 段落 | 起 | 迄 | 結束原因 | pid |
> |---|---|---|---|---|---|
> | 20260925083004 | open | 08:30:04 | 09:05:00 | 正常結束 | 74167 |
> | 20260925084004 | open | 08:40:04 | 09:05:00 | 正常結束 | 74585 |
> | 20260925132003 | close | 13:20:06 | 13:35:00 | 正常結束 | 90372 |
> | 20260925132803 | close | 13:28:03 | 13:50:00 | 正常結束 | 90706 |
> | 20260925143005 | after_close | 14:30:05 | 14:30:11 | 正常結束 | 93273 |
> | 20260925143504 | after_close | 14:35:04 | 14:35:08 | 正常結束 | 93497 |
>
> **Phase7-5 的修正在生產環境被證實，而且是在兩對重疊排程上各驗一次**：
> 開盤段 08:30 與 08:40 重疊到 09:05、尾盤段 13:20 與 13:28 重疊到 13:35——
> 四筆全部 `正常結束`，**沒有任何一筆被後啟動者標成崩潰**，
> `account_mode` 也全程停在 `NORMAL`（修正前這正是 halt 被靜默解除的路徑）。
>
> 其餘：`live_risk_event` 零筆、watchdog 零告警、盤後段產出 9 份報表
> （`__account__`／`__broker__`／`__unattributed__` 各 orders／positions／fills），
> `live_parity_diff` 零列（休市無訊號，`UNEXPLAINED = 0`）。
>
> **`update-db` 以結束碼 1 收尾**，是已知的 FinMind `register` 等級限制
> （券商分點無權限），16 個目標中 15 個成功，與本步驟無關。



### Phase7-5. 並行段落互相標記 CRASHED，並靜默解除帳戶層 halt ✅

- **目的**：`mark_previous_crash()` 的前提在實際排程下是錯的。它的 docstring 寫著

  > `ended_at IS NULL` 只有兩種可能：正在跑的這一次，或是上次崩潰了。

  **還有第三種：另一個正常執行中的段落。** SQL 是
  `WHERE ended_at IS NULL AND run_id != ?`——只排除自己，不排除別的活著的行程。

  而排程本來就**刻意重疊**：股票開盤段台北 08:30 啟動、09:05 收線，
  期貨開盤段 08:40 啟動；尾盤段 13:20 收線 13:35、期貨 13:28 啟動。兩對都重疊。

  **實測（2026-09-24 台北 08:40）**：

  ```
  live_run: 20260924083005  open  ended_at=08:40:07  end_reason='CRASHED'
  launchctl: 72026  com.alphaedge.live.stock-open   ← PID 當時仍存活
  ```

- **真正的後果不是誤報警，是帳戶層 halt 被靜默解除**：

  1. 股票開盤段啟動，插入 `live_run`，`account_mode` 是初始值 `NORMAL`；
     記憶體載入前次的 `REDUCE_ONLY`（它本段落沒有再降級，故 `_persist()` 不會被呼叫，
     那一列的 `account_mode` 一直是 `NORMAL`）。
  2. 期貨開盤段啟動 → 把前者標成 `CRASHED` 並**補上 `ended_at`**。
  3. 期貨開盤段接著 `load()` → `get_last_account_mode()` 只讀「已結束」的紀錄，
     於是讀到那一列的 **`NORMAL`**。

  日誌對照，一行不差：股票段有 1 行「讀回上次的帳戶層交易模式」，期貨段 **0 行**。

  `TradingModeState.load()` 的 docstring 明寫它存在就是為了防
  「不讀回就等於偷偷解除 halt——這是實盤最常見的事故型態」。
  **這條重疊路徑正好繞過它，而且連一行 warning 都沒有。**

- **第二個後果：停機狀態變成不確定的**。兩個段落同時收線時，
  `finish_run()` 各寫各的（一個 `REDUCE_ONLY`、一個 `NORMAL`），
  下一個段落讀到哪一個取決於誰後寫。

- **第三個後果**：期貨段在「以券商為準重建部位並對帳」時，股票段可能還在送單，
  兩者對同一個帳戶。

- **做法**（待裁示，三條都要一起考慮）：

  1. **崩潰判定要能分辨「還活著」**。可行的方向：`live_run` 增記 `pid` 與主機名，
     判定時以 `os.kill(pid, 0)` 確認行程是否存在；或改以「啟動時間早於本次、
     且超過該段落的最長可能時長」當門檻。**不可只靠 `ended_at IS NULL`。**
  2. **`get_last_account_mode()` 不該把 CRASHED 那列的 `account_mode` 當可信值**。
     崩潰的行程沒有機會寫下它最後的模式，那一格是插入時的初始值，
     不是「上次結束時的模式」。宜改為取「最近一筆**非** CRASHED 的紀錄」，
     或在標記 CRASHED 時**不要覆寫 `account_mode`**、另以獨立欄位記錄。
  3. **`AfterCloseRunner` 要套用 `resume_trading`**（見 Phase7-1 的 Day-2 紀錄）：
     目前 `--phase after_close --resume-trading` 靜默無效。

- **產出**：`core/dao/tw/live_trade_dao.py`、`core/live/trader.py`、`core/live/after_close.py`、
  `tests/live/`（新增測試：時窗重疊時不得誤標、CRASHED 紀錄不得清掉 halt、
  after_close 帶 resume 要生效）。

- **驗證方式**：模擬兩個 `live_run` 同時 `ended_at IS NULL` 時，後啟動者不標記前者；
  前次 `REDUCE_ONLY` 在崩潰紀錄存在時仍讀得回來；三條測試在修之前都要是紅的。

- **相依**：無。**建議優先於演練續跑**——它每天發作，且會讓演練期間的
  halt 保護形同虛設。
...
> **✅ 完成紀錄（2026-09-24）**
>
> **主要缺陷比規劃寫的更廣：不只重疊會觸發，任何一次崩潰都會。**
> 寫測試時才看清楚——`test_inherited_halt_survives_a_crash` 完全沒有重疊，
> 只是一個「繼承了降級的行程崩潰」，halt 照樣丟失。
> 根因是 `load()` 讀回 `REDUCE_ONLY` 後**從不回寫**：本段落若沒有再降級，
> `update_account_mode()` 就不會被呼叫，那一格停在插入時的 `NORMAL`。
> 這是 Phase5-3 當年修的那個漏洞的**殘留半邊**——當時修了「本次降級要即時落地」，
> 漏了「繼承來的降級也要落地」。
>
> **修好它之後，昨晚那個「195 毫秒決定安全狀態」的競態一併消失**：
> 被誤標的那列會帶著正確的 `REDUCE_ONLY`，兩個並行段落寫的值相同，誰後寫都一樣。
>
> **三個修法**（規劃列的三條，實作時第 2 條換了更根本的做法）：
> 1. `TradingModeState.load()` 讀回非 NORMAL 時即時落地。
> 2. `live_run` 增記 `pid`，`mark_crashed_runs()` 以 `os.kill(pid, 0)` 判定行程是否
>    仍存在，活著的跳過。**規劃原本建議改 `get_last_account_mode()` 不信 CRASHED 那列
>    的 `account_mode`**，但第 1 條修好之後那一格本來就是對的，不必再繞一層。
> 3. **恢復與讀回綁進同一個方法**，而不是在 `AfterCloseRunner` 再抄一份——
>    順序寫錯正是這個 bug 的成因之一。`TradingModeState` 自己持有 `resume_trading`，
>    `load()` 讀完就套用，任何呼叫端都不可能只做一半。
>
> **`pid` 欄位需要補欄位機制**：專案只有 `CREATE TABLE IF NOT EXISTS`，
> 對既有的表什麼都不做。新增 `_add_missing_columns()`，並在**正式紀錄庫的副本**上
> 實測：13 → 14 欄、13 筆舊列的 `pid` 皆為 NULL、重複呼叫無誤。
> 舊列（`pid` 為 NULL）一律視為崩潰——當成「還活著」的話，升級前留下的崩潰紀錄
> 會永遠標不起來。
>
> - **驗證**：`pytest -q -m "not slow"` 2369 passed / 15 skipped；分層違規 0；
>   文件路徑 0；孤兒方法 0；pre-commit 全過；回歸雙線逐筆相同。
> - **新增 `tests/live/test_concurrent_runs.py`（9 條）**，四個突變各自讓對應測試轉紅：
>   ① `load()` 不回寫；② 崩潰判定不看 pid；③ pid 為空當成還活著；④ `load()` 不套用 resume。
> - **正式環境確認**：修完後的實跑紀錄 `20260924144754` 已寫入 `pid=18289`。

---

### Phase7-6. 日線報價的 `date` 在實盤與回測型別不同 ✅

- **目的**：2026-09-24 演練尾盤段的實際崩潰——**第一次真正跑到策略鉤子，就掛在那裡**：

  ```
  ERROR | MomentumStrategy1 降級：鉤子執行失敗：
          can't compare datetime.datetime to datetime.date
  ```

  `BaseQuote.date` 宣告成 `Union[datetime.date, datetime.datetime]`，
  型別註解本身就允許兩種，於是兩條路徑給了不同的東西而沒有人發現：
  ...
  **回測一路綠燈，實盤在尾盤段當場掛掉並把策略降級為 HALTED，當日零委託。**
  開盤段踩不到：那裡是 `PreOpenStockQuote`，日期由本地時鐘給，本來就是 `date`。

  專案其實**已經有兩個正規化工具**（`TimeUtils.to_date()`、`normalize_quote_date()`），
  但兩者的 docstring 都寫「**tick 級別**的報價日期會是 datetime」——
  這個前提已經過時，實盤的**日線**快照也是。而 `MomentumStrategy1` 兩個都沒用。

- **做法**：三處取日期部分——`from_stock_snapshot()`、`from_futures_snapshot()`，
  以及期貨盤前報價（股票盤前本來就是 `self._now().date()`，
  同一個段落的兩個市場給出不同型別沒有理由）。

  **快照的精確時刻沒有遺失**：事件封包的 `ts` 記著它（盤中的報價過期判定看的就是那個，
  不是 `quote.date`），實盤委託另有 `created_at`。
  正規化反而讓 parity 更好——`StockOrder(date=signal.quote.date)` 會把型別一路傳到委託，
  而回測的委託本來就是 `date`。

- **產出**：`core/broker/tw/shioaji_quote_stream.py`、
  `core/live/datafeed/tw/futures_live_datafeed.py`、
  `tests/live/test_quote_date_parity.py`（新增）、`tests/live/test_shioaji_quote_stream.py`。

- **驗證方式**：2369 passed；回歸雙線逐筆相同（本步驟動 `core/broker/`，回測不該受影響）。

- **相依**：無。

> **✅ 完成紀錄（2026-09-24）**
>
> **既有的三條時戳測試改為直接斷言 `_resolve_date()`**：它們驗的是 `Snapshot.ts`
> 的奈秒與時區解碼（2026-09-22 模擬環境實測得來的硬知識），只是**釘錯了表面**——
> 對著 `quote.date` 驗的話，正規化之後時區與時分秒根本看不到。
> 另補一條測試釘住「不可以用丟掉時間來達成型別一致」，解碼本身仍要正確。
>
> - **新增 8 條測試**。三個突變（三處各改回 datetime）各自讓對應測試轉紅；
>   `test_strategy_can_compare_a_live_quote_date_with_its_calendar` **精確重現
>   正式環境的崩潰**（同例外、同檔案、同行號 163）。
>   型別斷言一律用 `type(...) is` 而非 `isinstance`——
>   `datetime.datetime` 是 `datetime.date` 的子類別，用 `isinstance` 等於沒有釘。
> - **修完後連模擬環境實跑確認**：策略掃完全市場並算出漲幅
>   （6925／6933／7547 各 10.0%、7825 9.89%、8121 9.86%、8201 9.92%），
>   `get_previous_trading_date()` 不再崩潰。送單時窗已過，`dispatch()` 在建 ticket
>   之前就擋下，沒有產生任何委託列。

---

### Phase7-2S. 正式環境小額上線檢查表（股票） ⬜

- **目的**：把「股票線上線前要確認的事」寫成可以逐項勾選的清單。
- **做法**：檢查表至少包含（兩線共通的部分兩份都要寫，不互相 include——
  上線時是照著單子逐項勾，跳到另一份會漏勾）：
  - CA 憑證位置與權限。
  - `RiskConfig` 上限調到小額。
  - kill switch 演練，以及 `TradingMode` 降級後重啟仍維持降級的驗證。
  - 通知管道實際送達測試、watchdog 已排程且和交易行程分開。
  - **首日只跑 `--dry-run` 並比對委託；次日才送真單。**
  - 監看 log 與 `live_risk_event` 的方式。

  股票線專屬：
  - 融資融券與現股當沖的資格檢查在正式帳戶實際生效（Phase6-1 只在模擬環境驗過）。
  - `__unattributed__` 的處理——正式帳戶若有本系統以外開的部位，第一次啟動會被收進去且只允許平倉。
  - 盤中零股**不支援**（Phase6-4 ⏸，下游全以張計），策略不可宣告零股。

- **產出**：`docs/live/production-checklist-stock.md`。
- **驗證方式**：使用者逐項勾選。
- **相依**：Phase0-1（已完成）、Phase7-1 的 **1S**。
  **無硬日期**——期貨線的 Phase6-6 卡 2026-10-20，本步驟不受它影響。

  **但上線節奏受共用程式碼約束**：見〈附：股票線與期貨線的分線〉的三段式建議。

### Phase7-2F. 正式環境小額上線檢查表（期貨） ⬜

- **目的**：同上，期貨線那一份。
- **做法**：共通部分同 Phase7-2S（逐字寫一份，不 include）。期貨線專屬：
  - 保證金檢查在正式帳戶的口徑（Phase6-5；模擬環境查不到帳務時走的是降級路徑，正式環境不會）。
  - **換月**：確認 `inject_roll_calendar()` 取得的未來交易日涵蓋下一個換月日，
    且換月當日的兩腿順序（先平舊月、再開新月）已由 Phase6-6 實跑驗證。
  - 股票期貨**不支援**（Phase6-3 ⏸，送單只解析指數期貨），策略不可宣告股期。
- **產出**：`docs/live/production-checklist-futures.md`。
- **驗證方式**：使用者逐項勾選。
- **相依**：Phase0-1（已完成）、Phase7-1 的 **1F**、Phase6-6 於 2026-10-20 實跑通過。
...
- **做法**：
  ...
  - `core/market/tw/market_calendar.py` 的 `check_shioaji_has_tick()` 會實際呼叫 Shioaji 查 tick，**不在本步驟範圍**，維持原樣。

    > 該函式已於 2026-09-22 刪除（零呼叫點），`market_calendar.py` 也因此不再 import shioaji。
- **產出**：上列檔案。
- **驗證方式**：`grep` 無殘留；`pytest` 通過；tick updater 冒煙一次。
- **相依**：Phase2-2。

> **✅ 完成紀錄（2026-09-22）**
> - **刪除**：`core/utils/order.py`（`OrderUtils`）、`core/utils/callback.py`（`Callback`）與 `core/utils/account.py` 的 `ShioajiAccount`（登入登出、`get_*_capital`／`get_*_pnl`／`generate_trade_record`），`core/utils/__init__.py` 同步。三者在刪除前都已沒有正式路徑的呼叫端，只剩登入登出被 tick 爬蟲使用。`ShioajiAPI`（金鑰容器）照規格保留給多帳號輪替，`account.py` 只剩它。
> - **`ShioajiSession` 補兩個參數**：`credentials=(api_key, secret_key)`（多帳號輪替逐組傳入，預設仍讀環境變數）、`verify_contract_date`（預設 True；只抓歷史資料的 ETL 關掉——長連假期間合約檔可能超過容許的 3 天沒更新，會擋住 tick 回補，而舊的登入沒有這道檢查）。新增模組函式 `login_read_only_sessions()`：每組金鑰各登入一次**正式環境、不啟用憑證**（送不出委託，只能查資料），失敗的帳號記錄後略過，與舊版「回 None 由呼叫端略過」的行為相同，但失敗訊息先經 session 刮掉憑證才進 log。
> - **遷移**：`StockTickUpdater`、`FuturesTickUpdater` 改持有 `sessions`（`api_list` 仍是給爬蟲的原生 API 物件），登出改 `session.close()`；`scripts/manual/manual_tick_crawler.py` 同步。`core/pipeline` 與 `core/broker` 同在分層 3，為同層不同套件的 import（檢查腳本只列出）。
> - 說明文字：指向已刪檔案的四段（`settings.py` 的循環相依例子、`shioaji_account_query.py` 與其測試的交割款形狀說明、`test_order_state_parity.py` 的回呼說明）改寫；歷史描述裡的 `OrderUtils` 改為泛稱，`grep -rn "OrderUtils\|Callback.order_callback"` 無結果。
> - 測試：`tests/live/test_shioaji_session.py` 新增 3 條（傳入金鑰優先於環境變數、關掉合約檔日期檢查時過期合約檔不擋、多帳號唯讀登入：正式環境、不啟用憑證、失敗的一組略過）。
> - 驗證：`uv run pytest` 2203 passed；ruff、三支檢查腳本、pre-commit 全綠；**tick 冒煙**：以 `login_read_only_sessions()` 登入正式環境（唯讀、不寫 `data/`），`StockTickCrawler` 抓 2330 在 2026-09-18 共 4,824 筆，欄位與 Shioaji 升級至 1.7 驗證 tick 爬取時（已完成並移出）相同。
...
**2026-09-25 裁示**：兩條線各自獨立宣告通過，不必互相等。

### 為什麼分

期貨線有一個**全年只有一次**的硬日期：Phase6-6 的換月兩腿要在 **2026-10-20 尾盤段**實跑
（台指期 10 月契約最後交易日 2026-10-21 的前一個交易日）。錯過要等 11/19。

股票線**沒有任何硬日期**——Phase6-1 只要盤中各送一筆融券、借券、當沖先賣。
不分線的話，股票線會被期貨的換月日拖住三週，而那三週它其實什麼都不缺。

### 怎麼分

| 線別 | 專屬步驟 | 阻塞 |
|---|---|---|
| 股票線 | Phase6-1（🔄）、Phase6-4（⏸ 零股）、Phase7-2S | Phase6-1 要盤中送單；Phase6-4 等有策略需要零股 |
| 期貨線 | Phase6-2（✅）、Phase6-3（⏸ 股期）、Phase6-5（✅）、Phase6-6（🔄）、Phase7-2F | Phase6-6 卡 2026-10-20；Phase6-3 等有股期策略 |
| 共用 | Phase0~Phase5、Phase7-1（兩線各驗一份）、Phase7-3~7-6 | — |

Phase4-9（成本校正）雖屬共用，但期貨那一側已於 2026-09-22 驗證一致（243 對 243），
**剩的只有股票側實跑**，故它實質上排在股票線。

### 分線解除了什麼、沒解除什麼

**解除了進度互卡**：行程本來就是分開的——每個交易日六個獨立行程
（股票／期貨 × 開盤／尾盤／盤後），期貨行程崩潰不會影響股票行程。

**沒有解除改動互相影響**：`LiveTrader`、`OrderManager`、`RiskManager`、`Reconciler`
是同一份程式碼。Phase7-5（並行段落互相標記 CRASHED）那個缺陷就是同時打到兩邊的。
`trader.py` 裡換月那 132 行，和送單、風控、對帳住在同一個檔案。

### 因此：股票上線建議分三段

直接讓股票線在 10 月初上線送真單，等於**拿真錢跑一份還在為期貨改動的程式**。
建議改為：

1. **股票線完成 backlog**（Phase6-1、Phase4-9 股票側、Phase7-1 的 1S、Phase7-2S）。
2. **正式環境跑 `--dry-run`**——零下單風險，但驗得到模擬環境驗不到的東西：
   正式憑證、正式行情、正式帳務欄位的實際值。這一段可以一路跑到期貨線收尾。
   順帶：這段累積的 `live_account_snapshot` 正是
   [暫緩工作彙整.md](暫緩工作彙整.md) S11 要的「約 20 個交易日」。
3. **共用程式碼凍結後才送真單**——也就是期貨線 2026-10-20 換月實跑通過之後。

第 2 段讓正式環境的實證提早三週開始，而不承擔「真錢跑在正在被改的程式上」的風險。

### 已知限制

- 兩份上線檢查表（Phase7-2S／7-2F）的共通項目**逐字各寫一份，不互相 include**。
  上線時是照單逐項勾，跳到另一份會漏勾。代價是共通項目改動要改兩處。
- 分線**不改變** Phase7-1 的「連續 5 個交易日」要求，只是兩線各自計算。
