...
| Phase6-5 | 送單前的期貨保證金檢查 | `core/live/trader.py`（或 `core/live/risk/`）、測試 | 可用保證金不足時不送單並寫事件；足夠時照常送出 | ⬜ | 2026-09-22 自 Phase6-2 拆出。相依 Phase6-2 |
| Phase6-6 | 期貨換月：平舊月＋開新月兩腿 | `core/live/trader.py`、測試 | 換月日模擬環境實跑一次；平倉腿未成交時開倉腿放棄並寫事件 | ⬜ | 2026-09-22 自 Phase6-2 拆出。相依 Phase6-2、Phase6-5 |
