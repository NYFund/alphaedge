...
| Phase6-3 | 股票期貨合約與乘數                                           | `core/broker/tw/shioaji_contract_resolver.py`                                                                                                                                             | 乘數取自合約並與 `futures_stock_universe` 交叉比對                                                                                                                     | ⏸ | 2026-09-22 暫緩：實盤的期貨送單只解析指數期貨，股期沒有送單路徑，也沒有策略交易股期。**解除條件：開始開發股期策略時**（與 [暫緩工作彙整.md](暫緩工作彙整.md) S4 同一個條件）。相依 Phase6-2 |
| Phase6-4 | 盤中零股                                                     | `core/models/stock/order.py`、`core/broker/tw/shioaji_order_mapper.py`                                                                                                                    | 數量單位錯配（張當股）在建立委託時就拋出                                                                                                                               | ⏸ | 2026-09-22 暫緩：零股建構檢查與送單前拒絕已做；下游（金額、成本、部位、歸屬帳、對帳）全以張為單位，回測也不支援零股。**解除條件：有策略需要零股時**，整條路徑一起改單位。相依 Phase2-4 |
...
| Phase7-1 | 模擬環境端到端演練（連續 5 個交易日）                        | 本文件（演練紀錄）                                                                                                                                                                        | 對應 Abstract 驗收標準 1、2、4、5                                                                                                                                      |  🔄  | **進行中**：Day-1（9/23）三段全部失敗當日作廢，Day-2（9/24）四個修正端到端驗證通過，9/25 中秋節休市日六段全部正常結束（跨休市日驗收達成）。9/29 起續跑。2026-09-22 演練前準備完成並以 launchd 排程啟動（`scripts/launchd/rehearsal_schedule.py`），演練交易日 9/23、9/24、9/29、9/30、10/1。相依 Phase4-6~Phase4-8、Phase5-5、Phase6-2、Phase6-5、Phase6-6 |
