...
- **做法**：在 `scripts/check_layer_deps.py` 做以下調整：
  - `_LAYER_RULES` 新增：
    ...
    - `("core.portfolio", 4, "部位建構層", False)`（2026-09-19 補，由 [策略分層重構.md](策略分層重構.md) S1 建立）：只可 import `core.models`／`core.utils`，**不可 import `core.backtest`／`core.live`／`core.strategies`**。跨策略仲裁的判定（`aggregation.py`）也在這一層。
...
- **相依**：Phase0-4；**[策略分層重構.md](策略分層重構.md) S5**（該份會先動 `core/strategies/` 與 `Backtester`，兩者都要跑回歸雙線，分開做才分得出變動成因）。
- **注意**：這是本文件唯一會動到回測引擎的步驟，建議單獨一個 commit，方便回退。
- **與分層重構的交界**：`max_holdings` 目前被 `EqualWeightSizer.size()`（訊號階段以候選數截斷）與 `Backtester.check_max_holdings()`（逐單階段檢查，可能含同一根 bar 稍早已開的倉）各檢查一次。**兩者不一定等價，落點尚未定案**——見[策略分層重構.md](策略分層重構.md) S6：第一件事是比對語意，確認等價才合併，不等價就兩處都留並寫明職責。本步驟先照現況把判定搬進 `order_preprocess`，**不順手合併那兩處**。
...
- **相依**：D1、D0、Phase1-2；**[策略分層重構.md](策略分層重構.md) S5**（本步驟與該份都會改 `BaseStrategy` 與 `ForeignSellShortDayTradeStrategy`，先分層後加實盤欄位，避免同一個方法改兩次）。
...
> **前置：[策略分層重構.md](策略分層重構.md) 要排在 Phase3-4 與 Phase4-1 之前**（2026-09-19 立項）。
