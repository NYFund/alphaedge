...
**三條路徑（股票回測、期貨回測、實盤券商快照）共用這一份**：規則只長在其中
一條上的話，另外兩條會再踩同一個坑——缺值變成 0 元報價、重複代號無人偵測——
而症狀都是「沒有錯誤訊息」。
...
def has_valid_price(close: Any) -> bool:
    ...
    - Description:
        ...
        **無成交日的 OHLC 是 NULL（舊資料裡則是 0）**：來源給的是 `--`，
        填成 0 就等於宣稱「當天成交價是 0 元」，回測會照著它成交。
        cleaner 保留 NULL，這裡把 NULL 與 0 兩種形態一起濾掉，
        讓尚未修復的資料庫也不會拿 0 元價去成交。
...
def resolve_close(raw: Any) -> Optional[float]:
    ...
    - Description:
        ...
        **回 `None` 而不是 0**：券商快照缺值時若退回 0，停牌或快照不完整的標的
        會變成一個「0 元」的報價物件——回測會濾掉這檔，實盤卻照樣拿去算訊號
