...
Sharpe 與 Sortino。reporter 輸出同樣的指標時一律呼叫這裡——同一個指標寫在兩個
地方，最後必然出現「報表說 1.2、前端說 0.8」而沒有人知道哪個對。
...
def compute_annualized_information_ratio(
    ...
    - Description:
        ...
        **樣本必須是時間序列、基準不可寫死 0**：以每筆交易的 `record.roi`
        為樣本無從年化；而從每一個樣本減去同一個常數不會改變分母，基準寫死 0
        時結果恆等於 `mean(每筆 ROI) / std(每筆 ROI)`——那是報酬的穩定性，
        與「相對基準的超額報酬」無關。
