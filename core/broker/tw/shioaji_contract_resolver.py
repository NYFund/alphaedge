...
把它往下傳給下單，錯誤訊息會完全指不到「代號打錯了」這個真正的原因。
...
**合約容器每次都要從 `api` 重取，不可存成屬性重用**。把它指派給變數之後，
拿到的是一份快照：`status` 會一直停在 `Fetching`，`Stocks.get()` 永遠回 `None`，
而且不會拋任何錯——查不到合約看起來就像「代號打錯」。
2026-09-22 模擬環境實測：同一個迴圈內每次重取 10/10 查得到，存起來的那份 0/10。
"""
