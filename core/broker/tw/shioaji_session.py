...
**連不上一律拋出**：登入失敗回傳 `None` 或旗標的話，呼叫端幾乎不會檢查，
「沒登入」會一路走到下單才爆。環境（`simulation`）與憑證（`activate_ca`）
都是建構參數，否則只連得到正式環境。
    ...
    **一個帳戶只開一個 session**：憑證是一組的，多重登入的行為未定義；
    ...
    def connect(self) -> None:
        ...
        - Description:
            ...
            **失敗一律拋出**，不回傳 `None` 或旗標：回值可以不檢查就往下走，
            於是「沒登入」會一路走到下單才爆，而那時的錯誤訊息完全指不到原因。
        - Raise:
            - RuntimeError
    ...
    def _fetch_contract_update_date(self) -> Optional[datetime.date]:
        ...
        **格式不只一種，而且猜錯的代價是整道檢查靜默失效**：只用
        `fromisoformat()` 解析的話，拿到 `'2026/09/21'` 會解析失敗被吞成 None，
        「本機日期差一天以上」那道守門從此不生效，而外顯的只有一行
...
def login_read_only_sessions(
    ...
    - Description:
        ...
        其餘照常：爬蟲本來就是多組輪替，一組不能用不該讓整輪停擺。
        失敗訊息經 `ShioajiSession` 刮掉憑證後才進 log。
    - Parameters:
        - credentials: Sequence[Tuple[str, str]]
            `[(api_key, secret_key)]`
    - Return:
