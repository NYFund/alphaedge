...
**為什麼需要這條路**：ETF 的受益權單位分割**不在任何結構化端點裡**。查遍
...
**那天它們都沒有任何公司行動**——單純是跌停，而只看 ±15% 的判準會把它們全部命中。

因此本模組**不把它們濾掉，而是標記**（`疑似槓桿反向ETF` 欄）：直接濾掉會連
真正的反向分割一起藏起來——00632R 在 2024-12-11 就做過一次反向分割。
...
# 槓桿／反向 ETF 的上限是 ±20%，單純跌停也會超過門檻，故只標記不濾掉
...
def detect_unexplained_moves(
    ...
    - Description:
        ...
        這同時是假跳空護欄（`tests/test_corporate_action_guard.py`）的判定式：
        事件表補齊後，這裡回傳的每一筆都應該要能被解釋掉；
        解釋不掉的就是新事件或資料錯誤。
    - Parameters:
...
def _drop_events_within_gap(
    ...
    - Description:
        ...
        跳空在 2015-01-07，差了 8 天。實測區間比對比逐日相等多解釋掉 24 筆
        （解釋不掉的候選由 361 降為 337）。
