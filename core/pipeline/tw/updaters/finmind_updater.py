...
本檔是**門面（facade）**：對外只暴露 `update_*()`，實作按資料集分別放在
`StockInfoUpdater`、`BrokerInfoUpdater`、`BrokerTradingUpdater`。
"""
...
class FinMindUpdater(BaseDataUpdater):
    ...
    def __init__(self) -> None:
        ...
        # **讀（股票／券商清單、metadata 重建）與寫（loader）共用同一條連線**：
        # 各開一條連線到同一個 DB 的話，updater 的 SELECT 會被 loader 的寫入鎖卡住，
        # 每次批次更新前都得先 commit 對方那條
    ...
    def update(
        ...
        **kwargs,
        ...
        """
        - Description:
            通用更新入口，依 `data_type` 分派到各資料集的更新流程
        - Parameters:
        ...
        # 字串一律轉成 Enum；大小寫皆接受，呼叫端不必記得正確拼法
    ...
    def update_all(
        ...
        # 券商分點是唯一需要日期區間的資料集，未指定時從最早可取得日起算
