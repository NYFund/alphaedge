...
以及 `pyproject.toml` 標為「潛在缺陷」的 `B006`／`B904`。
**修掉之後要把對應規則從 `ignore` 移除**，不要讓它長期留著——留著等於對那類問題
永久失明，而且「清單上有」會被誤讀成「還有那麼多處沒修」。`E722`（裸 `except`）
就是這樣歸零後整條移除的，本文件不再列它。

**數量一律以 `pyproject.toml` 的行尾註記為準，本文件不複製**：兩邊各記一份，
改了一邊就會分岔，而分岔的那份沒有任何東西會提醒。`tests/test_quality_ratchet.py`
會比對註記數量與實際處數，註記本身過期時它會紅。

逐處位置不寫在文件裡（行號會漂移），一律現查：

```bash
ruff check . --select BLE001 --statistics   # 看數量
ruff check . --select B006,B904             # 看逐處位置
```
