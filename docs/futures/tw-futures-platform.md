...
- **鏈式比對只記缺口、不拒收**：同一商品第 N 則公告的「調整前」應等於第 N−1 則的「調整後」（`check_margin_chain()`）。**每次 `--target futures_margin` 更新的收尾都會逐商品跑一次**（`log_margin_chain_gaps()`），斷點以 warning 列出生效日與前後值。不拒收的理由是：漏掉的那次調整可能根本不在抓得到的範圍內，拒收會讓這次真的抓到的資料也進不去。**看到斷點就要人工補登漏掉的公告**——那個區間的可開口數與追繳門檻都會偏掉，而且沒有執行期徵兆。
  對不上代表我們的歷史有缺口，不代表這一則有錯——拒收會讓一個缺口連鎖到後面所有公告。
  **已知斷點不重複告警**：2026-09-26 實查 60 個有公告紀錄的商品、共 10 處斷點（NYF、SRF、XBF、XIF、XJF 各一處，OAF／OBF／OJF／OKF／OOF 同為 2024-02-05），全部對應附件已被站方覆寫的公告，是來源本身的損失，登記在 `KNOWN_CHAIN_GAPS`。**以「商品 ＋ 生效日」逐筆登記而不是整商品跳過**——整商品跳過的話，同一商品日後真的漏抓一則公告就再也看不到。補回後要把該筆從清單刪掉。
...
| 滑價的跳動點 | 逐商品查 `FUTURES_TICK_SIZE`（TX／MTX／TMF 1 點、TE／ZEF 0.05 點、TF／ZFF 0.2 點） | 寫死 1 點會讓 TE 的滑價偏 20 倍；未登錄的商品退回 1 點並記 warning |
| 滑價的套用範圍 | 策略委託、**引擎強制出場**（追繳、到期兜底）與**換月轉倉的兩腿**，全部吃同一組 `fill_config` | 實盤換月是真的送兩張單、吃兩次價差；只算平倉腿會讓轉倉成本少一半 |
| `fill_config` 的型別 | 必須是 `FuturesFillConfig`，給基底 `FillConfig` 當場拋 `TypeError` | 型別標註擋不住：寫成 bps 照樣跑得完，只是假設默默換了一種 |

強制出場只取成交價、**不走 `fill()`**：券源檢核與成交量上限會拒單或縮量，
而強制出場被拒等於讓部位違規留倉。含滑價後超出當日區間時只警告並計入
`close_price_out_of_range`，**不夾回**（夾回會把成交價推向對持有者有利的一側）。
...
| **DolphinDB 的期貨 tick 寫入路徑未實測** | `--target futures_tick` 的爬取與清洗已驗證，入庫未驗證；無連線時保留中繼檔並記 warning | 啟動 DolphinDB server ＋ `uv sync --extra tick`，跑一天確認 |
...
| 對標序列在缺連續合約時退回近月拼接 | `futures_continuous` 沒建過該組（商品, 調整方式, 換月規則）時，對標曲線在換月接點有展期價差造成的假跳空；圖表註腳會標明採用哪一種 | 跑 `--target futures_continuous` 建出策略用的那一組換月規則（目前表內只有 `LAST_TRADING_DAY`） |
