...
**跨檔合併後整批寫入的 loader（dividend、corporate_action）沒有「整檔跳過」可言**：檔案先合併去重再一次
`INSERT OR REPLACE`，檔案與寫入沒有一對一關係。它們的摘要改報「讀取 N 檔、新增 M 列」（`finish_load(new_rows=...)`），
新增列數取寫入前後的列數差——覆寫既有主鍵不改變列數，重跑時 M 為 0。
...
**不在這條規則內，另有一套判準**：申報期內的部分申報（財報、月營收在申報期間只
拿得到已送件的公司）是「來源當下就只有這麼多」，不是市場失敗。判準是**申報期關沒關**：

| 來源 | 申報期限 | 判準 |
|------|----------|------|
| 財報三表、權益變動表 | 各季取各行業最晚那一天（Q1 5/30、Q2 8/31、Q3 11/29、Q4 次年 3/31） | `FinancialStatementUpdater.is_season_settled()` |
| 月營收 | 次月 10 日 | `MonthlyRevenueReportUpdater.is_month_settled()` |

兩者都再加 30 天寬限（逾期申報與申請延期都落在期限之後）。申報期尚未關閉的年季／
年月**不算收齊**，仍會留在候選裡重問；已入庫的列因為走 `INSERT OR IGNORE` 不會重複，
只有後送件的公司會被補進來。重問的代價很小——財報每張表每季只有上市、上櫃各一次請求。

**不這樣做的後果是永久缺口**：resume 是年季／年月差集，申報期內入庫的那一期一旦被
當成完成，之後就再也不會被請求，那一期從此只有先送件的那批公司，而且沒有任何徵兆。
