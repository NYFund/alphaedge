...
送降級事件、降級又會停止開新倉——後幾天會在降級狀態下空轉，
且「收盤後對帳零差異」在設計上就達不到。

**本工具不連券商、不寫任何東西**：`Reconciler._write_snapshots()` 一致時也會寫，
所以每次對帳都會落下 account／local／broker 三份快照。資料收集本來就是自動的，
這裡只負責讀出來回答那個問題。
