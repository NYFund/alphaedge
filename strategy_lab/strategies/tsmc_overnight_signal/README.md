...
- **交易成本扣法**：本研究有**兩條回測路徑，費率不同**，讀數字前要先確認在看哪一條。
  ...
  | 路徑 | 函式 | 手續費 | 證交稅 | 整張限制 | 最低手續費 | 用途 |
  |------|------|--------|--------|---------|-----------|------|
  | **主結果** | `run_backtest_with_signal()` | 0.1425% × 折扣 0.3 ＝ **0.04275%** | 0.3%（賣出） | **有** | **20 元** | `metrics_summary.csv`、報告中引用的所有數字 |
  | 診斷 | `run_vectorized_continuous_backtest()` | 同上（**由 `Commission` 推導**） | 0.3%（賣出） | 無 | 無 | 只用來對照「整張限制與最低手續費造成多少落差」 |

  **兩條路徑的費率現在是同一份**，都從 `core/utils/constant/cost.py` 的 `Commission` 推導，
  所以研究結論搬進生產不會因為費率而改變。

  > **2026-09-26 修正**：診斷路徑原本寫死 `0.1425%`（**未折扣**），與主結果差 3.33 倍。
  > 舊版把這當成刻意的，理由是「要看出整張買賣 ＋ 最低手續費的影響」——但費率不同會把
  > **折扣的影響一起混進來**，那個目的反而達不到。改成同一份費率後，兩條路徑的差額
  > 才真的只剩「整張限制 ＋ 最低手續費」。
  >
  > 診斷路徑的每次來回成本因此由 **0.5850% 降為 0.3855%**（少 0.1995 個百分點，
  > 約 34% 的成本降幅），該路徑輸出的診斷數字會比舊版好；**主結果不受影響**
  > （它本來就走 `StockUtils`）。
  >
  > 最低手續費在正規化權益模型裡無從適用：權益從 1.0 起算，沒有價格與張數，
  > 也就沒有「金額不足 20 元」這件事。這是模型的界線，不是漏掉。
